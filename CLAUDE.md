...
macOS and the repository is the only thing both see.
...
- **U+23F8 outside one terminal.** The `unicode` pause face carries default
  ...
  renders as text where it has been tried. `block` and `ascii` are the fallback
  sets. `no_face_risks_being_drawn_as_an_emoji` names it as the only permitted
  exception, so reaching for a neighbouring codepoint is a test failure and a
  fresh decision.

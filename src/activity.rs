...
CREATE INDEX IF NOT EXISTS journey_feedback_uri_idx
  ON journey_feedback(uri, created_at DESC);
...
/// Remember an explicit journey preference in permanent local activity data.
/// This is intentionally independent of network scrobbling and survives an
/// index rebuild.
pub fn record_journey_feedback(uri: &str, signal: i8) -> Result<()> {
    ensure_store()?;
    let conn = Connection::open(crate::paths::activity_file()?)?;
    conn.execute(
        "INSERT INTO journey_feedback(uri,signal,created_at) VALUES (?1,?2,?3)",
        params![uri, signal.clamp(-1, 1), crate::library::db::now_secs()],
    )?;
    Ok(())
}

...
/// Opt-in publication to the locally running Discord desktop client.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct DiscordPresence {
    pub enabled: bool,
    /// Application ID from a Discord developer application named STAR/AMP.
    pub client_id: String,
    /// Optional public profile button. This is separate from scrobbling
    /// credentials because a username is public and no secret is needed.
    pub lastfm_username: String,
}

/// Local inbox used to audition music before admitting it to the library.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct Imports {
...
/// The last Gravity Queue editor state. An anchor is deliberately not saved:
/// it names one occurrence in one queue, not a durable preference.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct Journey {
    pub preset: String,
...
pub struct Art {
    ...
    pub fetch: bool,
    /// Optional external image viewer command. The full-resolution image path
    /// is appended as the final argument; an empty list uses the desktop's
    /// default opener.
...
[imports]
# A separate inbox reviewed with `staramp import`. Rejected records are moved
# to quarantine; neither path may be inside the permanent library.
...
fetch = false
# Optional external viewer. Arguments are passed directly, never through a
# shell, and the full-resolution image path is appended last.
...
[discord]
# Publish the current track to the local Discord desktop client. Create an
# application named STAR/AMP at https://discord.com/developers/applications
# and paste its Application ID here. No bot or account token is required.
enabled = false
client_id = ""
...
[journey]
# Gravity Queue editor defaults. Choosing one only prepares it; shaping the
# upcoming queue remains an explicit action in the playlist's gravity panel.
preset = "off"
intensity = 0.75
artist_gap = 4
# Prefer breadth across the artists in a queue, from 0.0 (sonics alone) to 1.0.
artist_variety = 0.75
# Exclude weak seed matches instead of padding a Gravity list. Higher is stricter.
min_match = 0.70
seed = 1
...
mod tests {
    ...
    fn every_written_setting() -> Vec<(&'static str, &'static str, edit::Value)> {
        ...
        vec![
            ...
            ("playlist", "crossfade", Bool(true)),
            ("discord", "enabled", Bool(true)),
            ("journey", "preset", Str("wander".into())),
            ("journey", "intensity", Float(0.5)),
            ("journey", "artist_gap", Int(8)),
            ("journey", "artist_variety", Float(1.0)),
            ("journey", "min_match", Float(0.8)),
            ("journey", "seed", Int(42)),
    ...
    fn everything_the_player_writes_back_reads_back() {
        ...
        assert!(c.playlist.crossfade);
        assert!(c.discord.enabled);
        assert_eq!(c.journey.preset, "wander");
        assert!((c.journey.intensity - 0.5).abs() < 1e-3);
        assert!((c.journey.artist_variety - 1.0).abs() < 1e-3);
        assert!((c.journey.min_match - 0.8).abs() < 1e-3);
        assert_eq!((c.journey.artist_gap, c.journey.seed), (8, 42));
        assert!(c.eq.enabled);

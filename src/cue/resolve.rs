...
/// `FILE`s holding one track each. EAC writes a common malformed variant with
/// the next `TRACK` before its `FILE`: after parsing, that looks like two
/// tracks in the first stanza and none in the last, while there is still one
/// backing file per track.
///
...
pub fn is_per_track_cue(sheet: &CueSheet) -> bool {
    let file_count = sheet.files.len();
    file_count > 1
        && sheet.track_count() == file_count
        // A genuine multi-image sheet has at least one substantial stanza.
        // The malformed EAC form above has at most the two adjacent tracks
        // around a FILE boundary, which is still unambiguously one file per
        // recording and must not hide the individual encodes.
        && sheet.files.iter().all(|file| file.tracks.len() <= 2)
}
...
mod tests {
    ...
    #[test]
    fn eac_per_track_sheet_with_track_before_file_does_not_hide_encodes() {
        // This is the shape of Ratt's broken sheet: the following TRACK's
        // INDEX 00 is written before its own FILE stanza. The parser correctly
        // preserves the lines, but their physical placement must not turn the
        // separate FLACs into a fake disc image.
        let sheet = parse_str(
            "FILE \"01.wav\" WAVE\n\
             TRACK 01 AUDIO\nINDEX 01 00:00:00\n\
             TRACK 02 AUDIO\nINDEX 00 04:00:00\n\
        ...
        assert!(is_per_track_cue(&sheet));
    }

    #[test]
    fn multi_file_disc_image_is_not_per_track() {
        let sheet = parse_str(

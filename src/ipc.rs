...
fn handle_with_activity(
    ...
    match cmd {
        ...
        "crossfade" => format!("{}", player.crossfade()),
        ...
        "set-crossfade" => {
            let on = match rest.map(str::trim) {
                Some("true" | "on" | "1" | "yes") => true,
                None | Some("false" | "off" | "0" | "no") => false,
                _ => return "error: set-crossfade takes `on` or `off`".into(),
            };
        ...
        "set-journey-order" => {
            ...
            let request: Request = match rest.map(serde_json::from_str) {
                Some(Ok(request)) => request,
                _ => return "error: set-journey-order needs a JSON order".into(),
            };
            let mut queue = player.queue.lock().unwrap();
            if queue.revision() != request.revision {
                return "error: stale queue; shape the journey again".into();
            }
            if !queue.set_journey_order(request.order) {
                return "error: journey order is not a complete queue".into();
            }
            "ok".into()
        }
        "set-derived-list" => {
            ...
            struct Request {
                revision: u64,
                tracks: Vec<crate::playlist::queue::QueueItem>,
                #[serde(default)]
                album_grouped: bool,
            }
            let request: Request = match rest.map(serde_json::from_str) {
                Some(Ok(request)) => request,
                _ => return "error: set-derived-list needs a JSON track list".into(),
            };
            let mut queue = player.queue.lock().unwrap();
            if queue.revision() != request.revision {
                return "error: stale queue; create the Gravity list again".into();
            }
            if !queue.set_journey_tracks(request.tracks) {
                return "error: derived list is empty".into();
            }
            if request.album_grouped {
        ...
        "status" => {
            ...
            format!(
                r#"{{"state":"{state}","artist":{},"title":{},"position":{:.2},"duration":{:.2},"volume":{:.2},"crossfade":{},"bit_perfect":{}}}"#,
                json_string(&artist),
                json_string(&title),
                st.position_secs(),
                st.duration_secs(),
        ...
        "mirror" => {
            ...
            format!(
                r#"{{"state":"{}","artist":{},"title":{},"position":{:.2},"duration":{:.2},"volume":{:.3},"shuffle":{shuffle},"group":{group},"repeat":"{repeat}","crossfade":{},"index":{index},"total":{total},"revision":{revision},"rate":{},"depth":{},"channels":{},"codec":{},"bitrate":{},"bit_perfect":{},"uri":{},"view":{view_revision},"bands":"{bands}"}}"#,
                match st.state() {
                    PlayState::Playing => "playing",
                    PlayState::Paused => "paused",
                    PlayState::Stopped => "stopped",
...
mod tests {
    ...
    #[test]
    fn a_follower_can_replace_the_queue_with_a_seed_first_gravity_list() {
        ...
        assert_eq!(
            handle(
                &player,
                &crate::view::shared(),
                r#"set-queue ["old-a.flac","old-b.flac"]"#
            ),
            "2"
        );
        let revision = player.queue.lock().unwrap().revision();
        let tracks = ["seed.flac", "related.flac"]
            .into_iter()
            .map(|uri| QueueItem::new(TrackUri::parse(uri)))
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            handle(
                &player,
                &crate::view::shared(),
                &format!("set-derived-list {request}")
            ),
            "ok"
        );
        let queue = player.queue.lock().unwrap();
        assert_eq!(queue.len(), 2);
        assert_eq!(queue.current().unwrap().uri.to_string(), "seed.flac");
        assert_eq!(queue.view(), &[0, 1]);
    }

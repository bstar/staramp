...
impl Db {
    ...
    fn init(conn: Connection, activity_memory: bool) -> Result<Self> {
        ...
        match version {
            0 => conn.pragma_update(None, "user_version", schema::SCHEMA_VERSION)?,
            // SCHEMA is idempotently applied before this check. A v1 index did
            // not have the rebuildable sonic table, so it was just created in
            // its current form above.
            1 => {
                add_created_time(&conn)?;
                conn.pragma_update(None, "user_version", schema::SCHEMA_VERSION)?;
            }
            2 => {
                conn.execute_batch("ALTER TABLE sonic_feature ADD COLUMN distortion REAL;")?;
                add_created_time(&conn)?;
                conn.pragma_update(None, "user_version", schema::SCHEMA_VERSION)?;
            }
            3 => {
...
fn add_created_time(conn: &Connection) -> Result<()> {
    let present: bool = conn.query_row(
        "SELECT EXISTS(
           SELECT 1 FROM pragma_table_info('file') WHERE name = 'created_ns'
         )",
        [],
        |row| row.get(0),
    )?;
    if !present {
        conn.execute_batch("ALTER TABLE file ADD COLUMN created_ns INTEGER;")?;
    }
    Ok(())
}
    ...
    pub fn album_for_uri(&self, uri: &str) -> Result<Option<AlbumDetail>> {
        let row = self.conn.query_row(
            ...
            "SELECT t.album_id, t.album, COALESCE(t.album_artist, t.artist),
                    ...
                    COALESCE(t.genre, b.genre),
                    COALESCE(t.mb_release_id, b.mb_release_id),
                    COALESCE(t.mb_artist_id, b.mb_artist_id),
...
mod tests {
    ...
    #[test]
    fn version_two_sonic_rows_gain_the_distortion_column() {
        let conn = Connection::open_in_memory().unwrap();
        conn.execute_batch(
            "CREATE TABLE sonic_feature (
               uri TEXT PRIMARY KEY, file_size INTEGER NOT NULL,
               modified_at INTEGER NOT NULL, analyzer_version INTEGER NOT NULL,
               model_version TEXT NOT NULL, energy REAL, brightness REAL,
               dynamics REAL, tempo REAL, musical_key INTEGER, embedding BLOB,
               intro_embedding BLOB, outro_embedding BLOB,
        ...
        let columns: Vec<String> = db
            .conn
            .prepare("PRAGMA table_info(sonic_feature)")
            .unwrap()
            .query_map([], |row| row.get(1))
            .unwrap()
            .flatten()
            .collect();
        assert!(columns.iter().any(|name| name == "distortion"));
        let version: i32 = db
            .conn

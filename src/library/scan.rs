...
pub fn scan(db: &mut Db, root: &Path, opts: &ScanOptions) -> Result<ScanStats> {
    ...
    // An interrupted older scan could leave a `file` row behind without its
    // ordinary track row. A CUE sheet may then still create virtual tracks over
    // that file, which makes the index look populated even though selecting the
    // album reaches a malformed CUE entry. Treat those files as changed so a
    // normal incremental scan heals the direct audio row without a costly
    // `scan --force` over the entire library.
    let missing_plain_tracks = load_missing_plain_tracks(db)?;
        ...
        match existing.get(&f.rel_path) {
            Some(&(size, mtime))
                if !opts.force
                    && size == f.size
                    && mtime == f.mtime_ns
                    && !missing_plain_tracks.contains(&f.rel_path) =>
            {
                stats.unchanged += 1;
            }
            _ => changed.push(f),
...
fn walk(root: &Path) -> Vec<FoundFile> {
    ...
    for entry in walker.flatten() {
        ...
        out.push(FoundFile {
            ...
            created_ns: meta.created().ok().and_then(system_time_ns),
            kind,
        });
    }
    out
...
fn mtime_ns(meta: &std::fs::Metadata) -> i64 {
    meta.modified().ok().and_then(system_time_ns).unwrap_or(0)
}

fn system_time_ns(time: std::time::SystemTime) -> Option<i64> {
...
/// Audio files which have been discovered before but have no ordinary track
/// row. CUE virtual tracks intentionally share their `file_id`, so filtering on
/// `cue_ordinal IS NULL` is what distinguishes a missing direct track from a
/// healthy cue-backed one.
fn load_missing_plain_tracks(db: &Db) -> Result<HashSet<String>> {
            ...
            AND NOT EXISTS (
                SELECT 1
                 ...
                 WHERE t.file_id = f.id
                   AND t.cue_ordinal IS NULL
            )",
    )?;
    let rows = stmt.query_map([FileKind::Audio as i64], |r| r.get::<_, String>(0))?;
    Ok(rows.flatten().collect())
}
...
fn write_files(db: &mut Db, found: &[FoundFile], generation: i64) -> Result<()> {
    for chunk in found.chunks(BATCH) {
        ...
        for f in chunk {
            ...
            tx.execute(
                "INSERT INTO file
                    (dir_id, rel_path, size, mtime_ns, created_ns, kind, scan_gen)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)
                 ON CONFLICT(rel_path) DO UPDATE SET
                   size = ?3, mtime_ns = ?4, created_ns = COALESCE(file.created_ns, ?5),
                   kind = ?6, scan_gen = ?7",
                params![
                    did,
                    f.rel_path,
...
struct ExpandedCue {
    cue_rel: String,
    tracks: Vec<cue::expand::VirtualTrack>,
    suppress_backing: bool,
    per_track: bool,
    backing: Vec<String>,
}
...
fn write_cue_tracks(
    ...
    // `hidden` is derived entirely from the cue sheets seen in this scan.
    // Reset it before reapplying disc-image suppression so a sheet whose
    // interpretation changes (for example an EAC per-track cue previously
    // mistaken for an image) cannot leave its ordinary FLACs hidden forever.
    db.conn.execute("UPDATE track SET hidden = 0", [])?;
    ...
    let expanded: Vec<ExpandedCue> = cues
        .par_iter()
        .filter_map(|f| {
            ...
            let backing: Vec<String> = res
                ...
                .collect();
            Some(ExpandedCue {
                cue_rel: f.rel_path.clone(),
                tracks,
                suppress_backing: res.suppresses_backing_files(),
    ...
    for chunk in expanded.chunks(64) {
        let tx = db.conn.transaction()?;
        for cue in chunk {
            let cue_file_id: i64 = tx.query_row(
                "SELECT id FROM file WHERE rel_path = ?1",
                [&cue.cue_rel],
                |r| r.get(0),
            )?;
            ...
            for t in &cue.tracks {
                ...
                let uri = TrackUri::CueTrack {
                    cue_rel_path: cue.cue_rel.clone(),
                    ordinal: t.ordinal,
                }
                .to_string();
            ...
            // Per-track CUE rows are URI compatibility aliases for files which
            // already appear as ordinary tracks. Keep the aliases addressable
            // for old MPD playlists, but hide them from browsing and queue
            // construction so malformed sheets cannot make an album appear
            // twice. A disc image does the inverse: its virtual tracks are the
            // album, and only the large backing file is hidden.
            if cue.per_track {
                tx.execute(
                    "UPDATE track SET hidden = 1 WHERE cue_file_id = ?1",
                    [cue_file_id],
                )?;
            } else if cue.suppress_backing {
                for b in &cue.backing {

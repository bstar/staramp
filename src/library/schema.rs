...
-- Rebuildable local audio analysis. Embeddings are little-endian f32 vectors;
-- keeping the model and analyzer versions beside them makes upgrades targeted
-- rather than another reason to throw the whole index away.
CREATE TABLE IF NOT EXISTS sonic_feature (
  uri              TEXT PRIMARY KEY REFERENCES track(uri) ON DELETE CASCADE,
  ...
  musical_key      INTEGER,
  embedding        BLOB,
  intro_embedding  BLOB,
  outro_embedding  BLOB,
  analyzed_at      INTEGER NOT NULL
);
CREATE INDEX IF NOT EXISTS sonic_pending_idx
  ON sonic_feature(analyzer_version, model_version);

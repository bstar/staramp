...
enum Command {
    ...
    /// Review, audition, import or quarantine albums from a staging directory.
    Import {
        /// Inbox root. Defaults to `[imports] staging_root`.
        staging: Option<PathBuf>,
        /// Rejected albums are moved here. Defaults to `[imports] quarantine_root`.
        #[arg(long)]
    ...
    /// Analyze local audio for Gravity Queue journeys.
    Analyze {
        /// Report sonic-index coverage without decoding audio.
        #[arg(long)]
        status: bool,
        /// Analyze every pending track instead of a small foreground batch.
        #[arg(long)]
        all: bool,
        /// Maximum tracks for a foreground batch.
        #[arg(long, default_value_t = 25)]
        limit: usize,
    },
...
fn cmd_import(staging: Option<PathBuf>, quarantine: Option<PathBuf>) -> Result<()> {
    let cfg = config::Config::load()?;
    let library = cfg.require_library_root()?;
    let staging = staging.or(cfg.imports.staging_root.clone()).context(
        "no staging root configured — pass it to `staramp import` or set [imports] staging_root",
    )?;
    let quarantine = quarantine
        .or(cfg.imports.quarantine_root.clone())
        .unwrap_or_else(|| default_import_quarantine(&staging));

    // Import mode owns an audio player just like the ordinary TUI. Do not
    // quietly start a second leader and contend for the audio device or IPC
    // socket when STAR/AMP is already open elsewhere.
    anyhow::ensure!(
        mirror::Mirror::connect().is_none(),
        "STAR/AMP is already running — close it before opening import mode"
    );
    let socket = ipc::socket_path()?;
    anyhow::ensure!(
        ipc::claim_session(&socket),
        "another STAR/AMP instance started — close it before opening import mode"
    );

    eprintln!("scanning staging library at {}…", staging.display());
    let mut albums = library::import::discover(&staging, &library)?;
    anyhow::ensure!(
        !albums.is_empty(),
        "no albums found in {}",
        staging.display()
    ...
    let items = albums
        .iter_mut()
        .flat_map(|album| {
            let date = album.date_seconds;
            let year = date.map(epoch_year).map(i64::from);
            album.tracks.iter_mut().map(move |track| {
                track.timeline_date = date;
                track.timeline_year = year;
                track.clone()
            })
        })
        .collect();
...
fn default_import_quarantine(staging: &Path) -> PathBuf {
    let name = staging
        .file_name()
        .filter(|name| !name.is_empty())
        .map(|name| format!("{}-rejected", name.to_string_lossy()))
        .unwrap_or_else(|| "staramp-rejected".into());
    staging
        .parent()
...
#[allow(dead_code)]
fn cmd_import_repl(staging: Option<PathBuf>, quarantine: Option<PathBuf>) -> Result<()> {
    use std::io::Write;
    ...
    let quarantine = quarantine.or(cfg.imports.quarantine_root.clone()).context(
        "no quarantine root configured — pass --quarantine or set [imports] quarantine_root",
    )?;
    let mut filter = String::new();

    loop {
        println!("scanning staging library…");
        let albums = library::import::discover(&staging, &library)?;
        let shown: Vec<usize> = albums
            .iter()
            .enumerate()
            .filter(|(_, album)| {
                filter.is_empty()
        ...
        for (row, &index) in shown.iter().enumerate() {
            ...
            let date = album
                ...
                .unwrap_or_else(|| "????".into());
            let conflict = if album.existing.is_empty() {
                ""
            } else {
                "  [EXISTS]"
            };
            println!(
                "{:>4}. {date} · {} — {} · {} tracks{conflict}",
                row + 1,
                album.artist,
                album.title,
                album.tracks.len()
        ...
        println!();
        println!("p N audition · i N import · r N reject · / words filter · q quit");
        print!("> ");
        std::io::stdout().flush()?;
        ...
        let Some(row) = words.next().and_then(|word| word.parse::<usize>().ok()) else {
            eprintln!("choose an album number");
            continue;
        };
        let Some(&index) = row.checked_sub(1).and_then(|row| shown.get(row)) else {
            eprintln!("album number is outside the displayed list");
            continue;
        };
        let album = &albums[index];
        match command.to_ascii_lowercase().as_str() {
            "p" => audition_import_album(&staging, album, &cfg)?,
            "r" => {
                if !confirm(&format!(
                    "move {} — {} to quarantine",
                ...
                let moved = library::import::reject(&staging, &library, &quarantine, album)?;
                println!("quarantined at {}", moved.display());
            }
            "i" => {
                let artist_dir = choose_import_artist(album, &library)?;
                let conflict = if album.existing.is_empty()
                    && !artist_dir
                        .join(album.source_rel.file_name().unwrap_or_default())
                        .exists()
                {
                    library::import::Conflict::Beside
                } else {
                    println!("An apparent version of this album already exists:");
                    for path in &album.existing {
                        println!("  {}", path.display());
                    }
                    println!(
                        "b import beside it · x replace it (old copy is quarantined) · c cancel"
                    );
                    print!("> ");
                    std::io::stdout().flush()?;
                    ...
                    match answer.trim().to_ascii_lowercase().as_str() {
                        "b" => library::import::Conflict::Beside,
                        "x" => library::import::Conflict::Replace,
                        _ => continue,
                    }
                };
                if !confirm(&format!(
                ...
                println!("copying and verifying…");
                let done = library::import::import(
                    &staging,
                    &library,
                    &quarantine,
                    album,
...
fn choose_import_artist(album: &library::import::Album, root: &Path) -> Result<PathBuf> {
    use std::io::Write;
    if album.artist_candidates.is_empty() {
        let safe = album.artist.replace(['/', '\\'], "_");
        return Ok(root.join(safe));
    }
    println!("Artist directory candidates:");
    for (index, path) in album.artist_candidates.iter().enumerate() {
        println!("  {}. {}", index + 1, path.display());
    }
    println!(
        "  n. create {}/{}",
        root.display(),
        album.artist.replace(['/', '\\'], "_")
    );
    print!("> ");
    std::io::stdout().flush()?;
    let mut answer = String::new();
    ...
    if answer.trim().eq_ignore_ascii_case("n") {
        return Ok(root.join(album.artist.replace(['/', '\\'], "_")));
    }
    let chosen = answer
        .trim()
        .parse::<usize>()
        ...
        .and_then(|n| album.artist_candidates.get(n))
        .cloned();
    chosen.context("no artist directory selected")
}

fn audition_import_album(
    staging: &Path,
...
fn epoch_year(seconds: i64) -> i32 {
    ...
    let month = (5 * doy + 2) / 153 + 3;
    year += i64::from(month > 12);
    year as i32
}
...
fn cmd_analyze(status: bool, all: bool, limit: usize) -> Result<()> {
    let config = config::Config::load()?;
    let root = config.require_library_root()?;
    let mut db = library::db::Db::open(&index_path()?)?;
    sonic::run(&root, &mut db, status, all, limit)
}

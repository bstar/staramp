...
pub fn album_order(items: &[QueueItem], descending: bool, manual: &[String]) -> Vec<usize> {
    ...
    for (i, key) in keys.iter().enumerate() {
        ...
        g.timeline_date = g.timeline_date.or(items[i].timeline_date);
        g.members.push(i);
    }
...
struct Group {
    key: Option<Key>,
    year: Option<i64>,
    timeline_date: Option<i64>,
    members: Vec<usize>,
}

impl Group {
    fn bucket(&self) -> u8 {
        match (&self.key, self.timeline_date.or(self.year)) {
            (None, _) => UNTAGGED,
            (Some(_), None) => UNDATED,
            (Some(_), Some(_)) => DATED,
        }

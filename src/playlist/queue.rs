...
pub struct QueueItem {
    ...
    pub year: Option<i64>,
    /// Contextual chronology for a generated library timeline. This leaves
    /// the release year intact while allowing that derived list to group and
    /// label albums by a filesystem or index date.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub timeline_date: Option<i64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub timeline_year: Option<i64>,
...
impl Queue {
    ...
    /// Install an explicit journey order without moving the playing track.
    ///
    /// Journey planning names storage rows, as shuffle does. Refuse anything
    /// that is not a complete permutation: a partial smart result must never
    /// make tracks silently disappear from the queue or from a later save.
    pub fn set_journey_order(&mut self, order: Vec<usize>) -> bool {
        if !super::group::is_permutation(&order, self.tracks.len()) {
            return false;
        }
        let current = self.current_index();
        self.order = order;
        self.pos = current
            .and_then(|track| self.view_position(track))
            .unwrap_or(0);
        self.shuffle = false;
        self.group = None;
        self.manual.clear();
    ...
    /// Replace the queue with a seed-first subset selected from its current
    /// storage rows. Unlike a journey order, this intentionally permits a
    /// partial list: Gravity is generating a new playlist, not hiding rows in
    /// a source playlist that might later be overwritten.
    pub fn set_journey_selection(&mut self, selection: Vec<usize>) -> bool {
        ...
        if selection
            .iter()
            .any(|&index| index >= self.tracks.len() || !seen.insert(index))
        {
            return false;
        }
        let tracks = selection
            ...
            .collect();
        self.set_journey_tracks(tracks)
    }

    /// Replace the queue with a derived, seed-first Gravity list.
    ///
    /// Unlike `set_tracks`, this deliberately disables every ordering layer:
    /// the planner's first row is the base track and its remaining rows are
    /// already the intended playback path.
    pub fn set_journey_tracks(&mut self, tracks: Vec<QueueItem>) -> bool {
        if tracks.is_empty() {
            return false;
        }
        self.tracks = tracks;
    ...
    /// A successor that is safe to prepare as one continuous album stream.
    ///
    /// Merely appearing next in a playlist is not enough: pre-opening is an
    /// album-playback promise, so it is enabled only while album grouping is
    /// visibly controlling the order, with no play-next entry overriding it,
    /// and for adjacent tagged track numbers on the same record.
    pub fn album_gapless_successor(&self) -> Option<(usize, usize)> {
        if !self.grouped_now() || !self.play_next.is_empty() {
            return None;
        }
        let current = self.current_index()?;
        let next = self.peek_next()?;
        (current != next && album_tracks_are_adjacent(&self.tracks[current], &self.tracks[next]))
            .then_some((current, next))
    }
}

fn album_tracks_are_adjacent(current: &QueueItem, next: &QueueItem) -> bool {
    fn tag(value: Option<&str>) -> Option<String> {
        let value = value?.trim();
        (!value.is_empty()).then(|| value.to_lowercase())
    }

    let same_album = tag(current.album.as_deref())
        .zip(tag(next.album.as_deref()))
        .is_some_and(|(a, b)| a == b);
    let current_album_artist = tag(current
        .album_artist
        .as_deref()
        .or(current.artist.as_deref()));
    let next_album_artist = tag(next.album_artist.as_deref().or(next.artist.as_deref()));
    let same_artist = current_album_artist
        .zip(next_album_artist)
        .is_some_and(|(a, b)| a == b);
    let same_release = match (current.year, next.year) {
        (Some(a), Some(b)) => a == b,
        _ => true,
    };

    let (Some(current_track), Some(next_track)) = (current.track_no, next.track_no) else {
        return false;
    };
    let current_disc = current.disc_no.unwrap_or(1);
    let next_disc = next.disc_no.unwrap_or(1);
    let adjacent = (current_disc == next_disc && next_track == current_track.saturating_add(1))
        || (next_disc == current_disc.saturating_add(1) && next_track == 1);

    same_album && same_artist && same_release && adjacent
}
...
mod tests {
    ...
    #[test]
    fn a_journey_reorders_the_future_without_moving_the_playing_track() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(named(&["a", "b", "c", "d"]));
        q.jump_to(1);
        assert!(q.set_journey_order(vec![0, 1, 3, 2]));
        assert_eq!(q.current().unwrap().uri.to_string(), "b");
        assert_eq!(shown(&q), ["a", "b", "d", "c"]);
        assert!(!q.shuffled());
        assert_eq!(q.grouping(), None);
    }

    #[test]
    fn a_partial_journey_cannot_hide_tracks() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(named(&["a", "b", "c"]));
        assert!(!q.set_journey_order(vec![0, 2]));
        assert_eq!(shown(&q), ["a", "b", "c"]);
    }

    #[test]
    fn a_gravity_selection_becomes_a_real_seed_first_queue() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(named(&["a", "b", "c", "d"]));
        q.jump_to(2);
        assert!(q.set_journey_selection(vec![2, 0, 3]));
        assert_eq!(q.len(), 3);
        assert_eq!(q.view(), &[0, 1, 2]);
        assert_eq!(q.view_cursor(), 0);
        assert_eq!(q.current().unwrap().uri.backing_path(), "c");
    }
    ...
    fn album_track(name: &str, album: &str, artist: &str, disc: u32, track: u32) -> QueueItem {
        let mut item = QueueItem::new(TrackUri::parse(name));
        item.album = Some(album.into());
        item.album_artist = Some(artist.into());
        item.disc_no = Some(disc);
        item.track_no = Some(track);
        item.year = Some(2001);
        item
    }

    #[test]
    fn adjacent_album_tracks_are_gapless_candidates_only_in_album_order() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(vec![
            album_track("01.flac", "Record", "Band", 1, 1),
            album_track("02.flac", "Record", "Band", 1, 2),
        ]);
        assert_eq!(q.album_gapless_successor(), None);
        q.set_grouping(Some(false));
        assert_eq!(q.album_gapless_successor(), Some((0, 1)));
    }

    #[test]
    fn album_gapless_preparation_rejects_a_nonsequential_or_overridden_next_track() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(vec![
            album_track("01.flac", "Record", "Band", 1, 1),
            album_track("03.flac", "Record", "Band", 1, 3),
            album_track("other.flac", "Other", "Band", 1, 1),
        ]);
        q.set_grouping(Some(false));
        assert_eq!(q.album_gapless_successor(), None, "track 2 is absent");

        q.replace_at(1, album_track("02.flac", "Record", "Band", 1, 2));
        assert_eq!(q.album_gapless_successor(), Some((0, 1)));
        q.queue_next(2);
        assert_eq!(
            q.album_gapless_successor(),
            None,
            "play-next is an intentional break in album playback"
        );
    }

    #[test]
    fn the_first_track_of_the_next_disc_is_an_adjacent_album_boundary() {
        let mut q = Queue::with_seed(1);
        q.set_tracks(vec![
            album_track("disc1-last.flac", "Record", "Band", 1, 9),
            album_track("disc2-first.flac", "Record", "Band", 2, 1),
        ]);
        q.set_grouping(Some(false));
        assert_eq!(q.album_gapless_successor(), Some((0, 1)));
    }

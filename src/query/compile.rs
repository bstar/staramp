...
pub fn compile(q: &Query, now_epoch: i64, count_only: bool) -> Compiled {
    ...
    let needs_sonic = expr_needs_sonic(&q.filter)
        || q.sort.iter().any(|s| match s.key {
            SortKey::Field(f) => f.needs_sonic(),
            SortKey::Random => false,
        });
    ...
    if needs_stats {
        join.push_str(" LEFT JOIN activity.track_stat s ON s.uri = t.uri");
    }
    if needs_sonic {
        join.push_str(" LEFT JOIN sonic_feature sf ON sf.uri = t.uri");
    }
...
fn expr_needs_sonic(e: &Expr) -> bool {
    match e {
        Expr::All => false,
        Expr::Pred(p) => p.field.needs_sonic(),
        Expr::Not(inner) => expr_needs_sonic(inner),
        Expr::And(parts) | Expr::Or(parts) => parts.iter().any(expr_needs_sonic),
    }
}
...
fn column(f: Field) -> &'static str {
    ...
    match f {
        ...
        Energy => "COALESCE(sf.energy, 0)",
        Tempo => "COALESCE(sf.tempo, 0)",
        Brightness => "COALESCE(sf.brightness, 0)",
        Dynamics => "COALESCE(sf.dynamics, 0)",
        Distortion => "COALESCE(sf.distortion, 0)",
        Analyzed => "(sf.uri IS NOT NULL)",
    }
}
...
mod tests {
    ...
    #[test]
    fn sonic_fields_add_only_the_sonic_join() {
        let q = c("analyzed and energy > 0.7 sort bpm desc");
        assert!(q.needs_sonic);
        assert!(!q.needs_stats);
        assert!(q.sql.contains("LEFT JOIN sonic_feature sf"), "{}", q.sql);
        assert!(q.sql.contains("COALESCE(sf.energy, 0)"), "{}", q.sql);
        assert!(q.sql.contains("ORDER BY COALESCE(sf.tempo, 0) DESC"));
    }

    #[test]
    fn distortion_is_a_sortable_sonic_field() {
        let q = c("guitar > 0.65 sort distortion desc");
        assert!(q.needs_sonic);
        assert!(q.sql.contains("COALESCE(sf.distortion, 0) >"));
        assert!(q.sql.contains("ORDER BY COALESCE(sf.distortion, 0) DESC"));
    }

...
pub fn local_copy(host: &str) -> Result<PathBuf> {
    ...
    Ok(dir.join(copy_name(host)))
}

/// The file name a host's copy is kept under: a stable hash of the host, so
/// two hosts never share a copy and one host always finds its own.
///
/// Separate from [`local_copy`] so the naming can be tested without touching
/// the real cache directory, which a Nix build sandbox does not even have.
fn copy_name(host: &str) -> String {
    let h = blake3::hash(host.as_bytes());
    format!("{}.sqlite", &h.to_hex()[..16])
}
...
mod tests {
    ...
    fn every_host_gets_its_own_copy() {
        let a = copy_name("music");
        let b = copy_name("laptop");
        assert_ne!(a, b);
        assert_eq!(a, copy_name("music"), "and a stable one");

...
pub struct Session {
    ...
    /// Exact storage-index permutation produced by Gravity Queue. Kept compact
    /// because a large session is rewritten periodically while playing.
    #[serde(default)]
    pub journey_order: Vec<usize>,
    /// Fingerprint of the source queue's URI sequence. An edited playlist must
    /// not receive an old permutation that now points at different songs.
    #[serde(default)]
    pub journey_source_hash: u64,
    #[serde(default)]
    pub journey_preset: String,
    /// URI sequence of a filtered Gravity list. Unlike a permutation, this is
    /// enough to reconstruct a derived list whose discarded source rows are no
    /// longer in the live queue.
    #[serde(default)]
    pub journey_tracks: Vec<String>,
    pub saved_at: i64,
}
...
impl Session {
    ...
    pub fn describe_view(&self) -> Option<String> {
        ...
        let journey =
            (!self.journey_order.is_empty() || !self.journey_tracks.is_empty()).then(|| {
                format!(
                    "{} journey",
                    if self.journey_preset.is_empty() {
                        "gravity"
                    } else {
                        &self.journey_preset
                    }
        ...
        let parts: Vec<String> = [folded, arranged, journey].into_iter().flatten().collect();
...
mod tests {
    ...
    fn round_trips_through_toml() {
        let mut s = sample();
        s.journey_preset = "wander".into();
        s.journey_tracks = vec!["seed.flac".into(), "related.flac".into()];
        ...
        assert_eq!(back.journey_tracks, s.journey_tracks);
        assert_eq!(back.describe_view().as_deref(), Some("wander journey"));
    }
...
";
        ...
        assert!(s.journey_order.is_empty());
        assert!(s.journey_preset.is_empty());

...
impl Overlay {
    fn heading(self) -> &'static str {
        match self {
            ...
            Overlay::Filter => "SORTING",
            Overlay::Gravity => "GRAVITY",
...
enum PlaylistAddStep {
    ...
    TimelineSource,
    TimelineYear {
        source: crate::library::timeline::DateSource,
        years: Vec<i32>,
    },
...
struct PlaylistAddState {
    source: QueueItem,
    /// Position in the visible queue when the menu was opened. This identifies
    /// the exact occurrence when the same track appears more than once.
    view_index: usize,
...
struct ImportMode {
    ...
    library: PathBuf,
    quarantine: PathBuf,
    albums: Vec<crate::library::import::Album>,
    pending: Option<PendingImport>,
    running: Option<ImportJob>,
    operation: Option<String>,
    active_uris: Vec<String>,
}

struct ImportJob {
    result: std::sync::mpsc::Receiver<FinishedImport>,
    thread: std::thread::JoinHandle<()>,
}

enum FinishedImport {
    Imported(Result<crate::library::import::Imported, String>),
    Rejected(Result<PathBuf, String>),
}

enum PendingImport {
    Import {
        album: crate::library::import::Album,
        artist_dir: PathBuf,
        conflict: crate::library::import::Conflict,
    },
    Reject(crate::library::import::Album),
}
...
struct Remembered {
    ...
    repeat: String,
    crossfade: bool,
    journey_preset: String,
    journey_intensity: f32,
    journey_artist_gap: usize,
    journey_artist_variety: f32,
    journey_min_match: f32,
    journey_seed: u64,
    eq_enabled: bool,
    eq_preset: String,
    eq_preamp: f32,
...
impl Remembered {
    fn from_config(cfg: &crate::config::Config) -> Self {
        Self {
            ...
            discord_presence: cfg.discord.enabled,
            ...
            crossfade: cfg.playlist.crossfade,
            journey_preset: cfg.journey.preset.clone(),
            journey_intensity: cfg.journey.intensity,
            journey_artist_gap: cfg.journey.artist_gap,
            journey_artist_variety: cfg.journey.artist_variety,
            journey_min_match: cfg.journey.min_match,
            journey_seed: cfg.journey.seed,
...
/// Stable fingerprint of the queue a compact journey permutation describes.
fn journey_source_hash(items: &[QueueItem]) -> u64 {
    let mut hash = 0xcbf29ce484222325u64;
    for item in items {
        for byte in item.uri.to_string().bytes().chain(std::iter::once(0)) {
            hash ^= u64::from(byte);
...
/// Find a queue item in the canonical library pool, retaining an exact active
/// URI when the browser deliberately hides that alternate representation.
fn include_gravity_track(items: &mut Vec<QueueItem>, source: &QueueItem) -> usize {
    if let Some(index) = items.iter().position(|item| item.uri == source.uri) {
        index
    } else {
...
fn restored_journey_order(
    items: &[QueueItem],
    saved: &[usize],
    source_hash: u64,
) -> Option<Vec<usize>> {
    if saved.is_empty()
        || journey_source_hash(items) != source_hash
        || !crate::playlist::group::is_permutation(saved, items.len())
    {
        return None;
    }
    Some(saved.to_vec())
...
mod tests {
    use super::{
        bar_fraction, clamp_padding, codec_label, equalizer_enabled, g_prefix, hit,
        indicator_width, network_scrobblers, shifted_history_offset, slider_fraction, snap,
        status_indicators, step_toward, track_line, PrefixKey, KEY_VOLUME_STEP, MIN_HEIGHT,
        MIN_WIDTH, VOLUME_STEP,
    };
    use super::{
        journey_source_hash, restored_journey_order, resume_playlist_path, App, PlaylistAddState,
        PlaylistAddStep, PlaylistEntry,
    };
    use crate::playlist::queue::RepeatMode;
    use crate::vis::mode::VisMode;
    ...
    ///
    /// Recorded rather than reasoned about, because the overlay is the one
    /// screen whose whole job is to be read, and it was moved into starkit
    /// behind this test: the point is not that the layout is right but that it
    /// did not change when the code drawing it did. Colours are in the dump as
    /// well as characters, since a heading that stopped being a heading would
    /// pass a text-only comparison.
    ///
    ...
    fn the_help_overlay_is_drawn_as_recorded() {
        ...
        for (w, h, scroll) in [
            ...
            super::draw_help_overlay(&theme, scroll, area, &mut buf);
            out.push_str(&format!("== {w}x{h} scroll {scroll}\n"));
            out.push_str(&dump(&buf));
        }

        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("testdata/help-overlay.txt");
        if std::env::var_os("STARAMP_UPDATE_GOLDEN").is_some_and(|v| !v.is_empty()) {
            std::fs::write(&path, &out).expect("writing the recorded overlay");
            return;
        }
        ...
        if out != want {
            match out
                ...
                .find(|(_, (g, w))| g != w)
            {
                Some((n, (g, w))) => panic!("line {n} of the overlay reads\n{g}\nrecorded as\n{w}"),
                None => panic!(
                    "the overlay is {} lines, {} were recorded",
                    out.lines().count(),
                    want.lines().count()
                ),
            }
    ...
    /// A buffer as text, then the same grid as style letters with a legend.
    ///
    ...
    fn dump(buf: &starkit::ratatui::buffer::Buffer) -> String {
        ...
        for y in area.y..area.y + area.height {
            for x in area.x..area.x + area.width {
                ...
                let i = match styles.iter().position(|s| *s == style) {
                    Some(i) => i,
                    None => {
                        styles.push(style);
                        styles.len() - 1
    ...
    fn a_session(playlist: Option<&str>, name: &str) -> crate::session::Session {
        crate::session::Session {
            ...
            folded: Vec::new(),
            album_order: Vec::new(),
            journey_order: Vec::new(),
            journey_source_hash: 0,
            journey_preset: String::new(),
            journey_tracks: Vec::new(),
            saved_at: 0,
        }
    }
    ...
    fn only_two_plain_g_keys_go_to_the_first_visible_track() {
        ...
        assert_eq!(g_prefix(&mut pending, g), PrefixKey::Waiting);
        assert!(pending);
        assert_eq!(
            g_prefix(&mut pending, g),
            PrefixKey::Action(crate::ui::keymap::Action::Home)
        );
        assert!(!pending);

        assert_eq!(g_prefix(&mut pending, g), PrefixKey::Waiting);
        let j = KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE);
        assert_eq!(g_prefix(&mut pending, j), PrefixKey::None);
        assert!(!pending, "another command must cancel a lone prefix");
    }
    ...
    #[test]
    fn a_saved_journey_restores_its_exact_permutation() {
        use crate::playlist::queue::QueueItem;
        use crate::playlist::uri::TrackUri;

        let item = |uri: &str| QueueItem::new(TrackUri::parse(uri));
        let items = vec![item("a.flac"), item("same.flac"), item("same.flac")];
        let hash = journey_source_hash(&items);
        assert_eq!(
            restored_journey_order(&items, &[1, 0, 2], hash),
            Some(vec![1, 0, 2])
        );
    }

    #[test]
    fn an_edited_playlist_rejects_a_stale_journey_order() {
        use crate::playlist::queue::QueueItem;
        use crate::playlist::uri::TrackUri;

        let items = vec![QueueItem::new(TrackUri::parse("a.flac"))];
        assert_eq!(restored_journey_order(&items, &[0], 123), None);
    }
    ...
    fn the_status_bar_always_names_the_visualizer() {
        ...
        for m in VisMode::all() {
            let segs = status_indicators(*m, false, RepeatMode::Off, false);
            let vis = &segs[0].0;
            assert!(
    ...
    fn an_indicator_is_lit_by_its_own_state_only() {
        ...
        let segs = status_indicators(VisMode::Leds, false, RepeatMode::All, false);
        assert!(!segs[1].1, "shuffle is lit while off");
        assert!(segs[2].1, "repeat is not lit while on");

        let segs = status_indicators(VisMode::Off, true, RepeatMode::Off, false);
    ...
    fn the_reported_width_matches_what_is_drawn() {
        ...
        let segs = status_indicators(VisMode::Dots, true, RepeatMode::One, false);
    ...
    #[test]
    fn track_menu_offers_playlist_and_journey_actions() {
        let state = PlaylistAddState {
            source: crate::playlist::queue::QueueItem::new(crate::playlist::uri::TrackUri::parse(
                "song.flac",
            )),
            view_index: 7,
        ...
        let (_, _, rows) = App::playlist_add_rows(&state);
        assert_eq!(App::playlist_add_len(&state), 6);
        assert_eq!(rows[0].label, "add to playlist…");
        assert_eq!(rows[1].label, "remove from current playlist");
        assert_eq!(rows[2].label, "create Gravity list from this");
        assert_eq!(rows[3].label, "create playlist for this artist");
        assert_eq!(rows[4].label, "create library timeline…");
        assert_eq!(rows[5].label, "why is this track here?");
    }

    #[test]
    fn import_artist_choices_lead_with_the_useful_directory_name() {
        let mut source = crate::playlist::queue::QueueItem::new(
            crate::playlist::uri::TrackUri::parse("song.flac"),
        );
        source.artist = Some("Dünedain".into());
        let state = PlaylistAddState {
            source,
            view_index: 0,
            step: PlaylistAddStep::ImportArtist(vec![PathBuf::from(
                "/a/very/long/library/path/Terra Atlantica - Oceans",
            )]),
            cursor: 0,
            scroll: 0,
        };
        let (_, _, rows) = App::playlist_add_rows(&state);
        assert_eq!(rows[0].label, "use Terra Atlantica - Oceans");
        assert_eq!(rows[1].label, "create Dünedain");
    }

    #[test]
    fn import_artist_does_not_offer_a_duplicate_normalized_directory() {
        let mut source = crate::playlist::queue::QueueItem::new(
            crate::playlist::uri::TrackUri::parse("song.flac"),
        );
        source.artist = Some("Düne Dain".into());
        let state = PlaylistAddState {
            source,
            view_index: 0,
            step: PlaylistAddStep::ImportArtist(vec![PathBuf::from("/music/DÜNE-DAIN")]),
            cursor: 0,
            scroll: 0,
        };
        let (_, _, rows) = App::playlist_add_rows(&state);
        assert_eq!(App::playlist_add_len(&state), 2);
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].label, "use DÜNE-DAIN");
        assert_eq!(rows[1].label, "type artist directory…");
    }
}
...
struct Panels {
    ...
    playlist: bool,
    history: bool,
    /// First recent listen shown in the five-row Activity viewport.
...
/// Resolve Vim's two-key `gg` motion without swallowing the key after a lone
/// prefix. Any different key cancels the pending `g` and continues through the
/// ordinary keymap.
fn g_prefix(pending: &mut bool, key: KeyEvent) -> PrefixKey {
    let plain_g = key.code == KeyCode::Char('g') && key.modifiers == KeyModifiers::NONE;
    if !plain_g {
        *pending = false;
...
#[derive(Default)]
struct Editing {
    /// The first half of Vim's `gg` motion.
    g_prefix: bool,
...
#[derive(Default)]
struct JourneyUi {
    settings: crate::journey::Settings,
    /// Why a storage row occupies its current journey position.
    reasons: std::collections::HashMap<usize, String>,
    analyzed: usize,
    /// Set by the action and consumed only after one frame has rendered, so
    /// shaping a large queue never looks like an ignored key press.
    processing: bool,
}
...
struct Overlays {
    ...
    /// Held across opens: building it costs ~290 ms. `browse_model` compares
    /// its generation with the index before reusing it, so an external scan is
    /// visible on the next open without requiring an application restart.
...
pub struct App {
    ...
    /// Empty means the platform default. Otherwise the image path is appended
    /// to this argv without involving a shell.
    art_viewer: Vec<String>,
    ...
    /// Local, rebuildable feature analysis. Remote libraries are analyzed on
    /// the machine that owns their index, never by pulling whole files here.
...
impl App {
    ...
    fn open_track_menu(&mut self, source: QueueItem, view_index: usize) {
    ...
    /// Import mode acts on physical albums, never individual queue rows.
    fn open_import_album_menu(&mut self, mut source: QueueItem, view_index: usize) {
        let Some(mode) = &self.import else { return };
        if mode.pending.is_some() || mode.running.is_some() {
            return self.note("finish the current album operation first".into());
        }
        let Some(album) = Self::import_album(mode, &source) else {
            return self.note("that staging album is no longer present".into());
        };
        source.title = Some(format!("{} — {}", album.artist, album.title));
        self.panels.focus = Focus::Playlist;
        self.set_cursor(view_index);
        self.over.settings = None;
        self.over.playlist_add = Some(PlaylistAddState {
            source,
            view_index,
            step: PlaylistAddStep::ImportAlbum,
            cursor: 0,
            scroll: 0,
        });
    }
    ...
    fn import_artist_type(&mut self, key: starkit::crossterm::event::KeyEvent) -> bool {
        ...
        let PlaylistAddStep::ImportArtistName {
            candidates,
            mut value,
        } = state.step
        else {
        ...
        match key.code {
            ...
            KeyCode::Char(c) if !ctrl && c != '/' && c != '\\' && c != '\0' => value.push(c),
            KeyCode::Backspace => {
                value.pop();
            }
            KeyCode::Esc => {
                state.step = PlaylistAddStep::ImportArtist(candidates);
                state.cursor = Self::playlist_add_len(&state).saturating_sub(1);
                self.over.playlist_add = Some(state);
                return true;
            }
            KeyCode::Enter => {
                let Some(mode) = &self.import else {
                    return true;
                };
                match crate::library::import::artist_destination(&mode.library, &value) {
                    Ok(path) => {
                        state.step = PlaylistAddStep::ImportArtist(candidates);
                        return {
                            self.continue_import_to_artist(state, path);
                            true
                        };
                    }
                    Err(error) => {
                        self.note(format!("invalid artist directory: {error}"));
                    }
                }
            }
            _ => {
                state.step = PlaylistAddStep::ImportArtistName { candidates, value };
                self.over.playlist_add = Some(state);
                return false;
            }
        }
    ...
    fn import_artist_paste(&mut self, pasted: &str) -> bool {
        let Some(state) = &mut self.over.playlist_add else {
            return false;
        };
        let PlaylistAddStep::ImportArtistName { value, .. } = &mut state.step else {
            return false;
        };
            ...
            KeyCode::Enter => {
                ...
                if text != self.edit.words {
                    ...
                    if !self.edit.words.is_empty() {
                        // A filter is a fresh result set, not the tail below
                        // whichever track happened to hold the cursor. The
                        // rows rebuild on the next draw; zero lies before
                        // every possible match, so `nearest_shown` below moves
                        // it onto the first result without touching playback.
                        self.queue.cursor = 0;
                        self.queue.scroll = 0;
                    }
                }
    ...
    fn playlist_header(&self) -> Vec<crate::ui::panels::header::Item> {
        if self.import.is_some() {
            // This is an inbox, not an editable playlist. Its ordering and
            // membership describe staging albums and are deliberately fixed.
            return vec![crate::ui::panels::header::Item::Close];
        }
    ...
    /// Remove the exact queue occurrence whose context menu was opened.
    ///
    /// The URI check matters for a follower: its owner can change the queue
    /// while the modal is open, and a stale row number must not remove a new
    /// track that happened to move into that position.
    fn remove_context_track(&mut self, source: QueueItem, view_index: usize) {
        let protect = self.player.state.state() != PlayState::Stopped;
        let (still_there, kept) = {
            let q = self.player.queue.lock().unwrap();
            let still_there = q
                .view()
                .get(view_index)
                .and_then(|&storage| q.tracks().get(storage))
                .is_some_and(|track| track.uri == source.uri);
            let kept = protect && q.view_cursor() == view_index;
            (still_there, kept)
        };
        if !still_there {
            return self.note("the queue changed; open the track menu again".into());
        }

        let rows = [view_index];
        let n = match self.ask_session("remove-at", serde_json::json!({"rows": rows})) {
            Some(Ok(n)) => n,
            Some(Err(e)) => return self.note(format!("the session would not remove it: {e}")),
            None => self.player.queue.lock().unwrap().remove(&rows, protect),
        };
        if n == 0 {
            return self.note(if kept {
                "the playing track stays".into()
            } else {
                "the track was not removed".into()
            });
        }

        let title = source
            .title
            .unwrap_or_else(|| source.uri.backing_path().to_owned());
        self.after_edit();
        self.note(format!("removed {title}"));
    }
    ...
    fn with_player(
        ...
        let sonic = if player.vfs().is_remote() {
            None
        } else {
            player
                .vfs()
                .local_path("")
                .zip(player.vfs().index_path().ok())
                .map(|(root, index)| crate::sonic::Worker::spawn(root, index))
        };
        let discord = crate::discord::Handle::spawn(&cfg.discord, player.vfs().index_path().ok());
        ...
        let app = Self {
            ...
            journey: JourneyUi {
                settings: crate::journey::Settings {
                    preset: crate::journey::Preset::parse(&cfg.journey.preset),
                    intensity: cfg.journey.intensity.clamp(0.0, 1.0),
                    artist_gap: cfg.journey.artist_gap,
                    artist_variety: cfg.journey.artist_variety.clamp(0.0, 1.0),
                    min_match: cfg.journey.min_match.clamp(0.0, 1.0),
                    seed: cfg.journey.seed.max(1),
                    ..crate::journey::Settings::default()
                },
                ..JourneyUi::default()
            },
            import: None,
    ...
    fn poll_mirror(&mut self) {
        ...
        self.player.set_crossfade(st.crossfade);
    ...
    pub fn set_import_mode(
        ...
        // App::new normally assumes that its local root belongs to the main
        // index. In import mode the root is the staging directory instead:
        // point cover lookup at its private index and never let background
        // sonic analysis pair staged paths with rows in the permanent DB.
        self.sonic = None;
        if let Ok(index) = crate::paths::import_index_file() {
            self.art = crate::library::art::Watcher::spawn(
                index,
                Arc::clone(self.player.vfs()),
                Arc::clone(&self.art_fetch),
            );
        }
        self.import = Some(ImportMode {
            staging,
            library,
            quarantine,
            albums,
        ...
        queue.set_grouping(Some(false));
        drop(queue);
        self.panels.playlist = true;
        self.panels.focus = Focus::Playlist;
        self.queue.name = "IMPORT · oldest first".into();
    ...
    fn accept_resume(&mut self) {
        ...
        // A filtered Gravity list is its own queue, not a view permutation of
        // the source. Rebuild it from its compact URI sequence before looking
        // for the playing track.
        let mut restored = None;
        if !s.journey_tracks.is_empty() {
            let mut items: Vec<QueueItem> = s
                .journey_tracks
                .iter()
                .map(|uri| QueueItem::new(crate::playlist::uri::TrackUri::parse(uri)))
                .collect();
            crate::enrich_from_index(&mut items);
            self.player.set_queue_tracks(items);
            self.queue.name = s.playlist_name.clone();
            self.queue.source = None;
            restored = Some(true);
        } else if let Some(path) = resume_playlist_path(&s, &self.over.playlists) {
            match crate::load_playlist(&path) {
                Ok(items) if !items.is_empty() => {
                    self.player.set_queue_tracks(items);
                    self.queue.name = s.playlist_name.clone();
        ...
        let mut journey_restored = None;
        {
            ...
            if let Some(order) =
                restored_journey_order(q.tracks(), &s.journey_order, s.journey_source_hash)
            {
                if q.set_journey_order(order.clone()) {
                    journey_restored = Some(order);
                }
            }
        }
        ...
        if let Some(order) = journey_restored {
            self.journey.settings.preset = crate::journey::Preset::parse(&s.journey_preset);
            self.journey.reasons = order
                .into_iter()
                .map(|track| (track, format!("restored {} journey", s.journey_preset)))
                .collect();
        }
        ...
        // Open on the playing track and put it at the top of the viewport. The
        // queue order stays untouched, while the songs coming next remain in
        // view instead of being hidden above the bottom-anchored cursor.
        let playing = self.player.queue.lock().unwrap().view_cursor();
        let len = self.player.queue.lock().unwrap().len();
        self.queue.cursor = playing.min(len.saturating_sub(1));
        self.queue.scroll = usize::MAX;
    ...
    fn snapshot(&self) -> Option<Session> {
        ...
        let q_journey = if self.journey.reasons.is_empty() {
            Vec::new()
        } else {
            q.view().to_vec()
        };
        let q_journey_source_hash = if q_journey.is_empty() {
            0
        } else {
            journey_source_hash(q.tracks())
        };
        let q_journey_tracks = if self.journey.reasons.is_empty() || self.queue.source.is_some() {
            Vec::new()
        } else {
            q.view()
                .iter()
                .filter_map(|&track| q.tracks().get(track))
        ...
        Some(Session {
            ...
            journey_order: q_journey,
            journey_source_hash: q_journey_source_hash,
            journey_preset: self.journey.settings.preset.label().to_string(),
            journey_tracks: q_journey_tracks,
            folded: {
    ...
    fn save_session(&mut self, force: bool) {
        // The import inbox is a disposable view over a staging directory. It
        // must never replace the listener's resumable library/playlist state.
        if self.import.is_some() {
            return;
        }
    ...
    fn shutdown(&mut self) {
        ...
        if let Some(job) = self.import.as_mut().and_then(|mode| mode.running.take()) {
            // Do not terminate a filesystem transaction between installation,
            // indexing and removal of its staging source on graceful exit.
    ...
    fn event_loop(&mut self, term: &mut term::Tui) -> Result<()> {
        while !self.quit {
            ...
            if self
                .import
                .as_ref()
                .is_some_and(|mode| mode.pending.is_some())
            {
                self.run_pending_import();
                continue;
            }

            // Render the busy state before doing synchronous database and
            // planning work. The terminal then holds an honest progress
            // indicator instead of appearing frozen on the old screen.
            if self.journey.processing {
                self.journey.processing = false;
                self.shape_upcoming_tracks();
            ...
            if event::poll(FRAME)? {
                match event::read()? {
                    ...
                    Event::Paste(text) if self.import_artist_paste(&text) => {}
    ...
    fn settings_now(&self) -> Remembered {
        ...
        Remembered {
            ...
            listenbrainz: self
                ...
                .map(|p| p.enabled)
                .unwrap_or(false),
            discord_presence: self.discord.snapshot().enabled,
            ...
            journey_preset: self.journey.settings.preset.label().to_string(),
            journey_intensity: self.journey.settings.intensity,
            journey_artist_gap: self.journey.settings.artist_gap,
            journey_artist_variety: self.journey.settings.artist_variety,
            journey_min_match: self.journey.settings.min_match,
            journey_seed: self.journey.settings.seed,
    ...
    fn collect_settings(&mut self) {
        ...
        if now.discord_presence != was.discord_presence {
            set("discord", "enabled", Value::Bool(now.discord_presence));
        }
        ...
        if now.crossfade != was.crossfade {
            set("playlist", "crossfade", Value::Bool(now.crossfade));
        }
        if now.journey_preset != was.journey_preset {
            set("journey", "preset", Value::Str(now.journey_preset.clone()));
        }
        if now.journey_intensity != was.journey_intensity {
            set(
                "journey",
                "intensity",
                Value::Float(now.journey_intensity as f64),
            );
        }
        if now.journey_artist_gap != was.journey_artist_gap {
            set(
                "journey",
                "artist_gap",
                Value::Int(now.journey_artist_gap as i64),
            );
        }
        if now.journey_artist_variety != was.journey_artist_variety {
            set(
                "journey",
                "artist_variety",
                Value::Float(now.journey_artist_variety as f64),
            );
        }
        if now.journey_min_match != was.journey_min_match {
            set(
                "journey",
                "min_match",
                Value::Float(now.journey_min_match as f64),
            );
        }
        if now.journey_seed != was.journey_seed {
            set(
                "journey",
                "seed",
                Value::Int(now.journey_seed.min(i64::MAX as u64) as i64),
            );
        }
    ...
    fn playlist_add_len(state: &PlaylistAddState) -> usize {
        match &state.step {
            PlaylistAddStep::Menu => 6,
            PlaylistAddStep::ImportAlbum => 2,
            PlaylistAddStep::ImportArtist(candidates) => {
                let artist = state
                    .source
                    .album_artist
                    .as_deref()
                    .or(state.source.artist.as_deref())
                    .unwrap_or_default();
                candidates.len()
                    + usize::from(
                        !candidates
                            .iter()
                            .any(|path| crate::library::import::artist_dir_is_exact(artist, path)),
                    )
                    + 1 // manually typed directory
            }
            PlaylistAddStep::ImportArtistName { .. } => 1,
            PlaylistAddStep::ImportConflict(_) => 3,
            PlaylistAddStep::ImportReject => 2,
            PlaylistAddStep::TimelineSource => crate::library::timeline::DateSource::ALL.len(),
            PlaylistAddStep::TimelineYear { years, .. } => years.len() + 1,
    ...
    fn take_playlist_add(&mut self) {
        ...
        match state.step {
            PlaylistAddStep::ImportAlbum => match state.cursor {
                0 => self.choose_import_artist(state),
                1 => {
                    state.step = PlaylistAddStep::ImportReject;
                    state.cursor = 0;
                    self.over.playlist_add = Some(state);
                }
                _ => {}
            },
            PlaylistAddStep::ImportArtist(ref candidates) => {
                let Some(mode) = &self.import else { return };
                let Some(album) = Self::import_album(mode, &state.source) else {
                    return;
                };
                let exact_exists = candidates
                    .iter()
                    .any(|path| crate::library::import::artist_dir_is_exact(&album.artist, path));
                let create_at = candidates.len();
                let type_at = create_at + usize::from(!exact_exists);
                if state.cursor == type_at {
                    state.step = PlaylistAddStep::ImportArtistName {
                        candidates: candidates.clone(),
                        value: album.artist.clone(),
                    };
                    state.cursor = 0;
                    state.scroll = 0;
                    self.over.playlist_add = Some(state);
                ...
                let artist_dir = match candidates.get(state.cursor).cloned() {
                    Some(path) => path,
                    None if !exact_exists && state.cursor == create_at => {
                        mode.library.join(album.artist.replace(['/', '\\'], "_"))
                    }
                    None => return,
                };
                self.continue_import_to_artist(state, artist_dir);
            }
            PlaylistAddStep::ImportArtistName { .. } => {
                self.over.playlist_add = Some(state);
            }
            PlaylistAddStep::ImportConflict(ref artist_dir) => match state.cursor {
                0 => {}
                1 => self.apply_import_album(
                    state.source,
                    artist_dir.clone(),
                    crate::library::import::Conflict::Beside,
                ),
                2 => self.apply_import_album(
                    state.source,
                    artist_dir.clone(),
                    crate::library::import::Conflict::Replace,
                ),
                _ => {}
            },
            PlaylistAddStep::ImportReject => {
                if state.cursor == 1 {
                    self.apply_reject_album(state.source);
                }
            }
            PlaylistAddStep::Menu => match state.cursor {
                0 => {
                    self.over.playlist_add = Some(state);
                    self.open_playlist_destinations();
                }
                1 => self.remove_context_track(state.source, state.view_index),
                2 => self.create_gravity_from(state.source, state.view_index),
                3 => self.create_artist_playlist(state.source, state.view_index),
                4 => {
                    state.step = PlaylistAddStep::TimelineSource;
                    state.cursor = 0;
                    state.scroll = 0;
                    self.over.playlist_add = Some(state);
                }
                5 => self.explain_context_track(state.source, state.view_index),
                _ => {}
            },
            PlaylistAddStep::TimelineSource => {
                let Some(source) = crate::library::timeline::DateSource::ALL
                    .get(state.cursor)
                    .copied()
                else {
                    self.over.playlist_add = Some(state);
                ...
                let path = match self.player.vfs().index_path() {
                    Ok(path) => path,
                    Err(error) => {
                        return self.note(format!("cannot locate the library index: {error}"));
                    }
                };
                let db = match crate::library::db::Db::open_readonly(&path) {
                    Ok(db) => db,
                    Err(error) => {
                        return self.note(format!("cannot read the library index: {error}"));
                    }
                };
                let years = match crate::library::timeline::years(&db, &model, source) {
                    Ok(years) => years,
                    Err(error) => {
                        return self.note(format!("cannot build the library timeline: {error}"));
                    }
                };
                state.step = PlaylistAddStep::TimelineYear { source, years };
                state.cursor = 0;
                state.scroll = 0;
                self.over.playlist_add = Some(state);
            }
            PlaylistAddStep::TimelineYear { source, ref years } => {
                let year = state
                    .cursor
                    .checked_sub(1)
                    .and_then(|index| years.get(index).copied());
                self.create_timeline_playlist(source, year);
            }
            PlaylistAddStep::Destinations(ref entries) => {
                ...
                match destination.matches.len() {
                    0 => self.apply_playlist_destination(
                        destination,
                        None,
                        false,
                        state.source,
    ...
    /// Make the chosen row the base of a new full-library Gravity list using
    /// the criteria already selected in the Gravity panel.
    fn create_gravity_from(&mut self, source: QueueItem, view_index: usize) {
        let still_there = {
            let queue = self.player.queue.lock().unwrap();
            queue
                .view()
                .get(view_index)
                .and_then(|&storage| queue.tracks().get(storage))
                .is_some_and(|track| track.uri == source.uri)
        };
        if !still_there {
            return self.note("the queue changed; open the track menu again".into());
        ...
        self.set_cursor(view_index);
        let Some(anchor) = self.cursor_track() else {
            return self.note("that track is no longer in the queue".into());
        };
        self.journey.settings.anchor = Some(anchor);
        self.journey.settings.anchor_pull = 1.0;
        if self.journey.settings.preset == crate::journey::Preset::Off
            || self.journey.settings.preset == crate::journey::Preset::Bridge
        {
            self.journey.settings.preset = crate::journey::Preset::Wander;
        }
        self.apply_journey();
    }
    ...
    fn import_album<'a>(
        mode: &'a ImportMode,
        source: &QueueItem,
    ) -> Option<&'a crate::library::import::Album> {
        mode.albums
            .iter()
            .find(|album| album.tracks.iter().any(|track| track.uri == source.uri))
    }

    fn choose_import_artist(&mut self, mut state: PlaylistAddState) {
        let Some(mode) = &self.import else { return };
        let Some(album) = Self::import_album(mode, &state.source) else {
            return self.note("that staging album is no longer present".into());
        };
        state.step = PlaylistAddStep::ImportArtist(album.artist_candidates.clone());
        state.cursor = 0;
        state.scroll = 0;
        self.over.playlist_add = Some(state);
    }

    fn continue_import_to_artist(&mut self, mut state: PlaylistAddState, artist_dir: PathBuf) {
        let conflict = self
            .import
            .as_ref()
            .and_then(|mode| Self::import_album(mode, &state.source))
            .is_some_and(|album| {
                !album.existing.is_empty()
                    || artist_dir
                        .join(album.source_rel.file_name().unwrap_or_default())
                        .exists()
            });
        if conflict {
            state.step = PlaylistAddStep::ImportConflict(artist_dir);
            state.cursor = 0;
            state.scroll = 0;
            self.over.playlist_add = Some(state);
        } else {
            self.apply_import_album(
                state.source,
                artist_dir,
                crate::library::import::Conflict::Beside,
            );
    ...
    fn apply_import_album(
        ...
        let Some(album) = Self::import_album(mode, &source).cloned() else {
            return self.note("that staging album is no longer present".into());
        };
        if self
            .player
            .current_item()
            .is_some_and(|current| album.tracks.iter().any(|track| track.uri == current.uri))
        {
            self.activity.manual_end();
            self.command(&Command::Stop, "stop");
        }
        if let Some(mode) = &mut self.import {
            mode.operation = Some(format!("IMPORTING · {} — {}", album.artist, album.title));
            mode.active_uris = album
                .tracks
                .iter()
                .map(|track| track.uri.to_string())
                .collect();
            mode.pending = Some(PendingImport::Import {
                album,
                artist_dir,
                conflict,
            });
        }
        self.note("IMPORTING · copying and verifying album…".into());
    }

    fn run_pending_import(&mut self) {
        let Some(mode) = &mut self.import else { return };
        if mode.running.is_some() {
            return;
        }
        // Stop is handled by the decoder thread. Leave the operation pending
        // until it has acknowledged the command, so the source folder is
        // never moved out from under a track that is still playing.
        let removing_current = mode.pending.as_ref().is_some_and(|pending| {
            let album = match pending {
                PendingImport::Import { album, .. } | PendingImport::Reject(album) => album,
            };
            self.player
                .current_item()
                .is_some_and(|current| album.tracks.iter().any(|track| track.uri == current.uri))
        });
        if removing_current && self.player.state.state() != PlayState::Stopped {
            return;
        }
        let Some(pending) = mode.pending.take() else {
            return;
        };
        let staging = mode.staging.clone();
        let library = mode.library.clone();
        let quarantine = mode.quarantine.clone();
        let (tx, result) = std::sync::mpsc::channel();
        let thread = std::thread::Builder::new()
            .name("album-import".into())
            .spawn(move || {
                let finished = match pending {
                    PendingImport::Import {
                        ...
                        artist_dir,
                        conflict,
                    } => FinishedImport::Imported(
                        crate::library::import::import(
                            &staging,
                            &library,
                            &quarantine,
                            &album,
                    ...
                    PendingImport::Reject(album) => FinishedImport::Rejected(
                        crate::library::import::reject(&staging, &library, &quarantine, &album)
                            .map_err(|error| format!("{error:#}")),
                    ),
                };
        ...
        match thread {
            Ok(thread) => mode.running = Some(ImportJob { result, thread }),
            Err(error) => {
                mode.operation = None;
                mode.active_uris.clear();
                self.note(format!("could not start album import: {error}"));
            }
        }
    }
    ...
    fn poll_import(&mut self) {
        let finished = self
            .import
            .as_mut()
            .and_then(|mode| mode.running.as_mut())
        ...
        match finished {
            FinishedImport::Imported(Ok(done)) => {
                self.refresh_import_mode();
                self.note(format!(
                    "imported {} tracks to {}",
                    done.tracks,
                    done.destination.display()
                ));
            }
            FinishedImport::Imported(Err(error)) => {
                self.note(format!("album import failed: {error}"));
            }
            FinishedImport::Rejected(Ok(path)) => {
                self.refresh_import_mode();
                self.note(format!("moved album to {}", path.display()));
            }
            FinishedImport::Rejected(Err(error)) => {
                self.note(format!("could not quarantine album: {error}"));
            }
        }
    }

    fn apply_reject_album(&mut self, source: QueueItem) {
        let Some(mode) = &self.import else { return };
        if mode.pending.is_some() || mode.running.is_some() {
            return self.note("finish the current album operation first".into());
        }
        ...
        if let Some(mode) = &mut self.import {
            mode.operation = Some(format!("REJECTING · {} — {}", album.artist, album.title));
            mode.active_uris = album
                .tracks
                .iter()
                .map(|track| track.uri.to_string())
                .collect();
            mode.pending = Some(PendingImport::Reject(album));
        }
        self.note("REJECTING · moving album to quarantine…".into());
    }

    fn refresh_import_mode(&mut self) {
        let Some(mode) = &self.import else { return };
        let staging = mode.staging.clone();
        let library = mode.library.clone();
        match crate::library::import::discover(&staging, &library) {
            Ok(albums) => {
                let tracks: Vec<QueueItem> = albums
                    .iter()
                    .flat_map(|album| {
                        album.tracks.iter().cloned().map(|mut track| {
                            track.timeline_date = album.date_seconds;
                            track.timeline_year = album
                                .date_seconds
                                .map(crate::library::timeline::year_for_timestamp);
                            track
                        })
                    })
                    .collect();
                ...
                queue.set_grouping(Some(false));
                drop(queue);
                self.queue.cursor = self.queue.cursor.min(track_count.saturating_sub(1));
                self.queue.scroll = 0;
            }
            Err(error) => self.note(format!("could not refresh staging albums: {error:#}")),
        }
    }

    /// Create an unsaved, clicked-track-first list from every canonical track
    /// in the library credited to the same track or album artist.
    fn create_artist_playlist(&mut self, source: QueueItem, view_index: usize) {
        let active = {
            let queue = self.player.queue.lock().unwrap();
            ...
            still_there.then(|| {
                (
                    queue.revision(),
                    queue.current().is_some_and(|track| track.uri == source.uri),
                )
            })
        };
        let Some((revision, source_was_playing)) = active else {
            return self.note("the queue changed; open the track menu again".into());
        };
        let Some(artist) = source
            .artist
            .as_deref()
            .or(source.album_artist.as_deref())
            .map(str::trim)
            .filter(|artist| !artist.is_empty())
            .map(str::to_string)
        else {
            return self.note("this track has no artist metadata".into());
        };
        let model = match self.browse_model() {
            Ok(model) => model,
            Err(error) => return self.note(format!("cannot read the library index: {error}")),
        };
        let mut tracks = vec![source.clone()];
        tracks.extend(
            model
                .tracks
                .iter()
                .map(|track| &track.item)
                .filter(|item| item.uri != source.uri)
                .filter(|item| {
                    [item.artist.as_deref(), item.album_artist.as_deref()]
                        .into_iter()
                        .flatten()
                        .any(|candidate| candidate.trim().eq_ignore_ascii_case(&artist))
                })
                .cloned(),
        );

        if let Some(link) = &self.session.link {
            let request = serde_json::json!({"revision": revision, "tracks": tracks});
            match link
                .ask(&format!("set-derived-list {request}"))
                .as_deref()
                .map(str::trim)
            {
                Some("ok") => {}
                Some(error) => {
                    return self.note(format!(
                        "the session would not create the artist playlist: {}",
        ...
        let installed = {
            let mut queue = self.player.queue.lock().unwrap();
            queue.revision() == revision && queue.set_journey_tracks(tracks)
        };
        if !installed {
            return self.note("the queue changed while the artist playlist was built".into());
        }
        if !source_was_playing {
            self.activity.manual_end();
            self.command(&Command::PlayIndex(0), &format!("play-uri {}", source.uri));
        }
        self.journey.reasons.clear();
        self.queue.name = format!("{artist} — all tracks");
        self.queue.source = None;
        self.queue.cursor = 0;
        self.queue.scroll = 0;
        self.queue.dirty = true;
        self.note(format!(
            "artist playlist created · {count} tracks by {artist}"
        ));
    }

    /// Create an unsaved, album-grouped chronology from the canonical library.
    fn create_timeline_playlist(
        &mut self,
        source: crate::library::timeline::DateSource,
        year: Option<i32>,
    ) {
        ...
        let path = match self.player.vfs().index_path() {
            Ok(path) => path,
            Err(error) => return self.note(format!("cannot locate the library index: {error}")),
        };
        let db = match crate::library::db::Db::open_readonly(&path) {
            Ok(db) => db,
            Err(error) => return self.note(format!("cannot read the library index: {error}")),
        };
        let tracks = match crate::library::timeline::build(&db, &model, source, year) {
            Ok(tracks) if !tracks.is_empty() => tracks,
            Ok(_) => return self.note("no albums have that date".into()),
            Err(error) => return self.note(format!("cannot build the library timeline: {error}")),
        };
        if let Some(link) = &self.session.link {
                ...
                .map(str::trim)
            {
                ...
                Some(error) => {
                    return self.note(format!(
                        "the session would not create the timeline: {}",
                        error.trim_start_matches("error: ")
                    ));
                }
                None => return self.note("the session did not answer".into()),
        ...
        let installed = {
            let mut queue = self.player.queue.lock().unwrap();
            if queue.revision() != revision || !queue.set_journey_tracks(tracks) {
                false
            } else {
                queue.set_grouping(Some(false));
                true
        ...
        if !installed {
            return self.note("the queue changed while the timeline was built".into());
        }
        self.journey.reasons.clear();
        self.queue.name = match year {
            Some(year) => format!("Timeline · {year}"),
            None => "Library timeline".into(),
        };
        self.queue.source = None;
        self.queue.cursor = 0;
        self.queue.scroll = 0;
        self.queue.dirty = true;
        self.note(format!(
            "timeline created · {count} tracks · oldest albums first"
        ));
    }

    fn explain_context_track(&mut self, source: QueueItem, view_index: usize) {
        let track = {
            let queue = self.player.queue.lock().unwrap();
            queue.view().get(view_index).copied().filter(|&storage| {
                queue
                    .tracks()
                    .get(storage)
                    .is_some_and(|track| track.uri == source.uri)
        ...
        let explanation = self
            ...
            .get(&track)
            .cloned()
            .unwrap_or_else(|| "this row was not placed by the active journey".into());
        self.note(explanation);
    }
    ...
    fn handle(&mut self, action: Action) {
        ...
        if action == Quit
            && self
                .import
                .as_ref()
                .is_some_and(|mode| mode.pending.is_some() || mode.running.is_some())
        {
            return self.note("finish the current album operation before quitting".into());
        }
        ...
        if self.import.is_some()
            && (matches!(
                ...
                OpenFilter
                    ...
                    | ShuffleNow
                    | CycleRepeat
            ) || (action == OpenPanelSettings && self.panels.focus == Focus::Playlist))
        {
            return self.note(
                "import mode is fixed — right-click an album or track to import or reject it"
                    .into(),
            );
        }
        ...
        match action {
            ...
            ToggleCrossfade => {
                let on = !self.player.crossfade();
                if self.owns(&format!("set-crossfade {on}")) {
                    self.player.set_crossfade(on);
                }
                self.note(if on {
                    "crossfade on".into()
                } else {
            ...
            OpenCover => self.open_cover(),
            PrevCover => self.cycle_cover(-1),
            NextCover => self.cycle_cover(1),
            ToggleAlbumPanel => {
    ...
    fn play_track_at_cursor(&mut self) {
        ...
        if uri.as_ref().is_some_and(|uri| {
            self.import
                .as_ref()
                .is_some_and(|mode| mode.active_uris.contains(uri))
        }) {
            return self.note("that album is being removed from staging".into());
        }
        ...
        // On the next draw, make the selected track the first visible row so
        // the upcoming queue is visible. This changes only the viewport.
        self.queue.scroll = usize::MAX;
    }
    ...
    /// Move the five-row Activity viewport through the retained recent log.
        ...
        match m.kind {
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = r.playlist {
                    ...
                    match header::hit(rect, &words, x, y) {
                        ...
                        Some(header::Item::Gravity) => return self.open_gravity(),
                ...
                if let Some(rect) = r.album {
                    match header::hit(rect, header::PLAIN, x, y) {
                        ...
                        Some(header::Item::Filter | header::Item::Gravity) => {}
                    ...
                    if hit(rect, x, y) {
                        ...
                        let fallback_album = item.as_ref().and_then(|i| i.album.as_deref());
                        let fallback_artist = item.as_ref().and_then(|i| i.artist.as_deref());
                        ...
                        if let Some((previous, next)) = album::cover_controls(
                            rect,
                            self.show_cover(),
                            self.cell_aspect(),
                            album.as_deref(),
                        ...
                        {
                            return self.open_catalog(crate::library::art::CatalogTarget::Album);
                        }
                        if album::catalog_rect(
                            rect,
                            self.show_cover(),
                        ...
                        .is_some_and(|r| hit(r, x, y))
                        {
                            return self.open_catalog(crate::library::art::CatalogTarget::Artist);
                        }
                        return;
                    }
                }
                ...
                if let Some(rect) = r.history {
                    match header::hit(rect, header::PLAIN, x, y) {
                        Some(header::Item::Close) => {
                            ...
                            self.panels.focus = Focus::Player;
                            return self.note("activity closed — alt+s to bring it back".into());
                        }
                ...
                if hit(r.status, x, y) {
                    return self.status_click(r.status, x);
                }
                if hit(r.player, x, y) {
            ...
            MouseEventKind::Down(MouseButton::Right) => {
                if let Some(rect) = r.playlist {
                    if self.import.is_some() {
                        let index = self
                            .playlist_album_at(rect, x, y)
                            .or_else(|| self.playlist_track_at(rect, x, y));
                        if let Some(index) = index {
                            if let Some(source) = self.queue.items.get(index).cloned() {
                                self.open_import_album_menu(source, index);
                            }
                        }
                        // Never fall through to the ordinary per-track menu in
                        // the staging inbox, even if a stale row could not be
                        // resolved after a refresh.
                        return;
                    }
                    if let Some(index) = self.playlist_track_at(rect, x, y) {
                        ...
                        if let Some(source) = source {
                            ...
                            return self.open_track_menu(source, index);
                        }
                    }
                }
    ...
    fn playlist_album_at(&self, rect: Rect, x: u16, y: u16) -> Option<usize> {
        let inner = playlist::list_rect(rect);
        if !hit(inner, x, y) {
            return None;
        }
        let row = self.queue.scroll + (y - inner.y) as usize;
        match self.queue.rows.rows().get(row) {
            Some(playlist::Row::Section { .. }) => self.queue.rows.track_for_section(row),
            _ => None,
        }
    }
    ...
    /// Handle the persistent controls in the footer. The hit boxes mirror the
    /// renderer below so a click always acts on the label under the pointer.
    fn status_click(&mut self, area: Rect, x: u16) {
        let (shuffled, repeat) = {
            let q = self.player.queue.lock().unwrap();
            (q.shuffled(), q.repeat())
        };
        let segments = status_indicators(self.vis.mode, shuffled, repeat, self.player.crossfade());
        let ind_w = indicator_width(&segments);
        let help = Rect::new(area.x + 1, area.y, 6.min(area.width.saturating_sub(1)), 1);
        if hit(help, x, area.y) {
            return self.handle(Action::Help);
        }
        if area.width <= ind_w + 4 {
            return;
        }

        let mut next_x = area.x + area.width - ind_w - 1;
        for (i, (label, _)) in segments.iter().enumerate() {
            if i > 0 {
                next_x += SEPARATOR.chars().count() as u16;
            }
            let rect = Rect::new(next_x, area.y, label.chars().count() as u16, 1);
            if hit(rect, x, area.y) {
                return self.handle(match i {
                    0 => Action::ToggleVisualizer,
                    1 => Action::ToggleShuffle,
                    2 => Action::CycleRepeat,
                    3 => Action::ToggleCrossfade,
                    _ => return,
                });
            }
            next_x += rect.width;
    ...
    fn playlist_add_rows(state: &PlaylistAddState) -> (&'static str, String, Vec<settings::Row>) {
        ...
        match &state.step {
            PlaylistAddStep::ImportAlbum => (
                "IMPORT ALBUM",
                track,
                vec![
                    settings::Row::action("import whole album…"),
                    settings::Row::action("reject whole album…"),
                ],
            ),
            PlaylistAddStep::ImportArtist(candidates) => {
                let mut rows: Vec<settings::Row> = candidates
                    .iter()
                    .map(|path| {
                        let directory = path
                            .file_name()
                        ...
                        // Absolute library paths all share a long prefix and
                        // hide the only part the listener needs to choose.
                        settings::Row::action(format!("use {directory}"))
                    })
                    .collect();
                let artist = state
                    .source
                    ...
                    .as_deref()
                    .or(state.source.artist.as_deref())
                    .unwrap_or("album artist");
                if !candidates
                    .iter()
                    .any(|path| crate::library::import::artist_dir_is_exact(artist, path))
                {
                    rows.push(settings::Row::action(format!("create {artist}")));
                }
                rows.push(settings::Row::action("type artist directory…"));
                ("IMPORT TO ARTIST", track, rows)
            }
            PlaylistAddStep::ImportArtistName { value, .. } => (
                "TYPE ARTIST",
                track,
            ...
            PlaylistAddStep::ImportConflict(_) => (
                "ALBUM EXISTS",
                track,
                vec![
                    settings::Row::action("cancel — keep existing"),
                    settings::Row::action("import beside existing"),
                    settings::Row::action("replace existing (quarantine old)"),
                ],
            ),
            PlaylistAddStep::ImportReject => (
                "REJECT ALBUM",
                track,
                vec![
                    settings::Row::action("cancel"),
                    settings::Row::action("move whole album to quarantine"),
                ],
            ),
            PlaylistAddStep::Menu => (
                ...
                track,
                vec![
                    settings::Row::action("add to playlist…"),
                    settings::Row::action("remove from current playlist"),
                    settings::Row::action("create Gravity list from this"),
                    settings::Row::action("create playlist for this artist"),
                    settings::Row::action("create library timeline…"),
                    settings::Row::action("why is this track here?"),
                ],
            ),
            PlaylistAddStep::TimelineSource => (
                "LIBRARY TIMELINE",
                "choose the date assigned to each album".into(),
                crate::library::timeline::DateSource::ALL
                    .into_iter()
                    .map(|source| settings::Row::action(source.label()))
                    .collect(),
            ),
            PlaylistAddStep::TimelineYear { source, years } => {
                let mut rows = vec![settings::Row::action("all years")];
                rows.extend(
                    years
                        .iter()
                        .map(|year| settings::Row::action(year.to_string())),
                );
                ("LIBRARY TIMELINE", source.label().into(), rows)
            }
    ...
    fn draw_playlist(&mut self, area: Rect, buf: &mut starkit::ratatui::buffer::Buffer) {
        ...
        if let Some(operation) = self
            .import
            .as_ref()
            .and_then(|mode| mode.operation.as_deref())
        {
            const SPINNER: [&str; 8] = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧"];
            name = format!("{} {operation}", SPINNER[self.look.marquee % SPINNER.len()]);
        }
    ...
    fn settings_for(&self, panel: Focus, kind: Overlay) -> (String, Vec<(settings::Row, Setting)>) {
        ...
        if kind == Overlay::Filter {
            return self.sorting_for();
        }
        if kind == Overlay::Gravity {
            return self.gravity_for();
        }
        ...
        match panel {
            Focus::Album => (
                ...
                vec![
                    ...
                    (Row::action("open full image"), Setting::OpenCover),
                    (
                        Row::action("open artist database"),
                        Setting::OpenArtistCatalog,
                    ),
                    (
                        Row::action("open album database"),
                        Setting::OpenAlbumCatalog,
                    ),
            ...
            Focus::History => {
                ...
                let discord = self.discord.snapshot();
                let mut rows = Vec::new();
                ...
                let discord_state = if !discord.configured {
                    "needs app id"
                } else if discord.enabled && discord.connected {
                    "on · connected"
                } else if discord.enabled && discord.error.is_some() {
                    "on · unavailable"
                } else if discord.enabled {
                    "on · waiting for Discord"
                } else {
                    "off"
                };
                rows.push((
                    Row::setting("Discord presence", discord_state),
                    Setting::DiscordPresence,
                ));
                rows.extend([
                    (
                ...
                ("listening activity and scrobbling".into(), rows)
            }
    ...
    fn sorting_for(&self) -> (String, Vec<(settings::Row, Setting)>) {
        use settings::Row;
        let q = self.player.queue.lock().unwrap();
    ...
    /// Shape the future queue from listening and sonic relationships.
    ///
    /// Gravity is deliberately separate from sorting: sorting applies an
    /// ordinary, predictable order, while Gravity plans a path through it.
    fn gravity_for(&self) -> (String, Vec<(settings::Row, Setting)>) {
        use settings::Row;
        let rows = vec![
            (
                Row::setting(
                    ...
                    format!(
                        "{}{}",
                        self.journey.settings.preset.label(),
                        match self.journey.settings.anchor_pull {
                            pull if pull > 0.0 => " · toward selected",
                            pull if pull < 0.0 => " · away from selected",
                            _ => "",
                        }
                    ),
                ),
                Setting::JourneyPreset,
            ),
            (
                Row::setting(
                    "journey pull",
                    format!("{}%", (self.journey.settings.intensity * 100.0).round()),
                ),
                Setting::JourneyIntensity,
            ),
            (
                Row::setting(
                    "artist gap",
                    format!("{} tracks", self.journey.settings.artist_gap),
                ),
                Setting::JourneyArtistGap,
            ),
            (
                Row::setting(
                    "artist variety",
                    format!(
                        "{}%",
                        (self.journey.settings.artist_variety * 100.0).round()
                    ),
                ),
                Setting::JourneyArtistVariety,
            ),
            (
                Row::setting(
                    "minimum match",
                    format!("{}%", (self.journey.settings.min_match * 100.0).round()),
                ),
                Setting::JourneyMinMatch,
            ),
            (
                Row::setting("journey seed", self.journey.settings.seed.to_string()),
                Setting::JourneySeed,
            ),
            (
                Row::action(if self.journey.processing {
                    "⠋ building related list…"
                } else {
                    "create related list"
                }),
                Setting::JourneyApply,
            ),
            (
                Row::action(format!(
                    "why this track?{}",
                    if self.journey.reasons.is_empty() {
                        " — no active journey"
                    } else {
                        ""
                    }
    ...
    /// Open the playlist's Gravity Queue controls, or close them if they are
    /// already open.
    fn open_gravity(&mut self) {
        self.panels.playlist = true;
        self.panels.focus = Focus::Playlist;
        self.open_overlay(Focus::Playlist, Overlay::Gravity)
    }

    /// Build a new, seed-first list from related tracks in the current queue.
    /// The source playlist remains untouched until the listener explicitly
    /// saves the derived list somewhere.
    fn apply_journey(&mut self) {
        if self.journey.processing {
            return;
        }
        if self.journey.settings.preset == crate::journey::Preset::Off {
            self.journey.reasons.clear();
            return self.note("choose a journey in gravity first".into());
        }

        if self.journey.settings.preset == crate::journey::Preset::Bridge {
            let Some(anchor) = self.cursor_track() else {
                return self.note("select the track the bridge should land on".into());
            };
            let playing = self.player.queue.lock().unwrap().current_index();
            if Some(anchor) == playing {
                return self.note("select a different destination for the bridge".into());
            }
            self.journey.settings.anchor = Some(anchor);
        }

        self.journey.processing = true;
        self.refresh_settings();
        self.note("⠋ building related list…".into());
    }

    fn shape_upcoming_tracks(&mut self) {
        let index = match self.player.vfs().index_path() {
            Ok(path) => path,
            Err(error) => return self.note(format!("cannot find the library index: {error}")),
        };
        let db = match crate::library::db::Db::open_readonly(&index) {
            Ok(db) => db,
            Err(error) => return self.note(format!("cannot read the library index: {error}")),
        };
        let active = {
            let q = self.player.queue.lock().unwrap();
            q.current().cloned().map(|playing| {
                let anchor = self
                    .journey
                    .settings
                    .anchor
                    .and_then(|track| q.tracks().get(track))
                    .cloned();
                (playing, anchor, q.revision())
            })
        };
        let Some((playing_item, anchor_item, revision)) = active else {
            return self.note("play a track to use as the Gravity base".into());
        };

        // Gravity always derives from the full canonical library, regardless
        // of which playlist happens to be open. An active alternate encoding
        // that the canonical browser model hides is appended explicitly so it
        // can remain the exact first/playing track.
        let model = match self.browse_model() {
            Ok(model) => model,
            Err(error) => return self.note(format!("cannot read the library index: {error}")),
        };
        let mut items: Vec<QueueItem> = model
            .tracks
            .iter()
            .map(|track| track.item.clone())
            .collect();
        let current = include_gravity_track(&mut items, &playing_item);
        let mut settings = self.journey.settings.clone();
        settings.anchor = anchor_item
            .as_ref()
            .map(|anchor| include_gravity_track(&mut items, anchor));
        let order: Vec<usize> = (0..items.len()).collect();

        // A bounded priority pass analyzes up to 25 missing members before
        // idle library work, making a later application richer without
        // blocking the UI on analysis.
        if let Some(worker) = &self.sonic {
            worker.request_tracks(items.iter().map(|item| item.uri.to_string()).collect());
        }
        let features = crate::journey::load_features(&db.conn, &items);
        let plan = crate::journey::plan(
            &items,
            &order,
            current,
            &features,
        ...
        let seed = plan.order.first().copied().unwrap_or(current);
        let seed_item = items[seed].clone();
        let selected = plan.order.len();
        let removed = items.len().saturating_sub(selected);
        let matched = plan.match_quality.len();
        let average_quality = (matched > 0)
            .then(|| plan.match_quality.values().copied().sum::<f32>() / matched as f32);
        let lowest_quality = plan.match_quality.values().copied().reduce(f32::min);
        let selected_items: Vec<QueueItem> = plan
            .order
            .iter()
            .map(|&track| items[track].clone())
            .collect();
        let remapped_reasons = plan
            .order
            .iter()
            .enumerate()
            .filter_map(|(new, old)| plan.reasons.get(old).cloned().map(|reason| (new, reason)))
            .collect();

        if let Some(link) = &self.session.link {
            let request = serde_json::json!({"revision": revision, "tracks": selected_items});
            match link
                .ask(&format!("set-derived-list {request}"))
                .as_deref()
                .map(str::trim)
            {
                ...
                Some(error) => {
                    return self.note(format!(
                        "the session would not shape the journey: {}",
                        error.trim_start_matches("error: ")
                    ))
                }
                None => return self.note("the session did not answer".into()),
        ...
        let installed = {
            ...
            if queue.revision() != revision {
                ...
                queue.set_journey_tracks(selected_items)
            }
        };
        if !installed {
            return self.note("the queue changed while the journey was being planned".into());
        }
        if seed_item.uri != playing_item.uri {
            self.activity.manual_end();
            let uri = seed_item.uri.to_string();
            self.command(&Command::PlayIndex(0), &format!("play-uri {uri}"));
        }
        self.journey.reasons = remapped_reasons;
        self.journey.analyzed = plan.analyzed;
        self.journey.settings.anchor = None;
        self.journey.settings.anchor_pull = 0.0;
        let seed_name = seed_item
            .title
            .as_deref()
            .unwrap_or_else(|| seed_item.uri.backing_path());
        self.queue.name = format!("{} from {seed_name}", self.journey.settings.preset.label());
        self.queue.source = None;
        self.queue.cursor = 0;
        self.queue.scroll = 0;
        self.queue.dirty = true;
        let quality = match (average_quality, lowest_quality) {
            (Some(average), Some(lowest)) => format!(
                "{matched} good matches · quality avg {:.0}% / low {:.0}%",
                average * 100.0,
                lowest * 100.0
            ),
            _ => format!(
                "no matches met the {:.0}% minimum",
                self.journey.settings.min_match * 100.0
            ),
        };
        self.note(format!(
            "{} list created · {selected} tracks · {quality} · {removed} filtered out · {}/{} analyzed",
            self.journey.settings.preset.label(),
            self.journey.analyzed,
            items.len()
        ));
    }

    fn explain_journey_track(&mut self) {
        let Some(track) = self.cursor_track() else {
            return;
        };
        let explanation = self
    ...
    fn take_setting(&mut self) {
        ...
        match item {
            ...
            Setting::JourneyPreset => {
                self.journey.settings.preset = self.journey.settings.preset.next();
                self.journey.settings.anchor = None;
                self.journey.settings.anchor_pull = 0.0;
                if self.journey.settings.preset == crate::journey::Preset::Off {
                    self.journey.reasons.clear();
                ...
                self.refresh_settings();
                self.note(format!("journey: {}", self.journey.settings.preset.label()));
            }
            Setting::JourneyIntensity => {
                self.journey.settings.intensity = match self.journey.settings.intensity {
                    v if v < 0.38 => 0.5,
                    v if v < 0.63 => 0.75,
                    v if v < 0.88 => 1.0,
                    _ => 0.25,
            ...
            Setting::JourneyArtistGap => {
                self.journey.settings.artist_gap = match self.journey.settings.artist_gap {
                    0 => 2,
                    1..=3 => 4,
                    4..=7 => 8,
                    _ => 0,
            ...
            Setting::JourneyArtistVariety => {
                self.journey.settings.artist_variety = match self.journey.settings.artist_variety {
                    v if v < 0.13 => 0.25,
                    v if v < 0.38 => 0.5,
                    v if v < 0.63 => 0.75,
                    v if v < 0.88 => 1.0,
            ...
            Setting::JourneyMinMatch => {
                self.journey.settings.min_match = match self.journey.settings.min_match {
                    v if v < 0.68 => 0.70,
                    v if v < 0.73 => 0.75,
                    v if v < 0.78 => 0.80,
                    v if v < 0.83 => 0.85,
            ...
            Setting::JourneySeed => {
                self.journey.settings.seed = self.journey.settings.seed.wrapping_add(1).max(1);
                self.refresh_settings();
            }
            Setting::JourneyApply => {
                return self.apply_journey();
            }
            Setting::JourneyWhy => return self.explain_journey_track(),
            ...
            Setting::DiscordPresence => {
                let status = self.discord.snapshot();
                if !status.configured {
                    return self.note(
                        "create a Discord application named STAR/AMP, then set discord.client_id in config.toml"
                            .into(),
                    );
                }
    ...
    fn open_cover(&mut self) {
        let Some(uri) = self.current_uri() else {
            return self.note("nothing playing to open artwork for".into());
        };
        let Some(w) = &self.art else {
            return self.note("no library artwork is available".into());
        };
        if w.album_for(&uri).and_then(|a| a.image.clone()).is_none() {
            return self.note("this album has no image to open".into());
        }
        if w.open_image(&uri, &self.art_viewer) {
            self.note("opening full-resolution artwork\u{2026}".into());
        } else {
            self.note("artwork worker is busy — try again".into());
        }
    }

    fn open_catalog(&mut self, target: crate::library::art::CatalogTarget) {
        let Some(uri) = self.current_uri() else {
            return self.note("nothing playing to look up".into());
        };
        let Some(w) = &self.art else {
            return self.note("no library metadata is available".into());
        };
        if w.open_catalog(&uri, target) {
            self.note(match target {
                crate::library::art::CatalogTarget::Artist => {
                    "looking up the artist\u{2026}".into()
                }
                crate::library::art::CatalogTarget::Album => "looking up the album\u{2026}".into(),
            });
        } else {
            self.note("album lookup worker is busy — try again".into());
        }
    }

    fn poll_art_outcome(&mut self) {
        let message = self.art.as_ref().and_then(|w| w.outcome());
    ...
    fn draw_status(&self, area: Rect, buf: &mut starkit::ratatui::buffer::Buffer) {
        ...
        // The footer is reserved for stable state. Transient notes belong in
        // the interaction flow; this affordance is always present and opens
        // the same modal as the `?` key.
        buf.set_string(
            area.x + 1,
            area.y,
            "? help",

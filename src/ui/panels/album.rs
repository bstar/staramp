...
/// Click targets for the album and artist database links. They are derived
/// from the same centred rows the renderer uses.
pub fn catalog_rect(
    area: Rect,
    show_cover: bool,
    cell_aspect: f32,
    ...
    if !matches!(kind, Kind::Title | Kind::Artist) {
        return None;
    }
    let g = geometry(area, show_cover, cell_aspect)?;
    let rows = lines(album, fallback_album, fallback_artist);
    let top = g.text.y + (g.text.height.saturating_sub(rows.len() as u16)) / 2;
    let (index, (text, _)) = rows.iter().enumerate().find(|(_, (_, k))| *k == kind)?;
    let width = text.chars().count().min(g.text.width as usize) as u16;
    (width > 0).then_some(Rect::new(g.text.x, top + index as u16, width, 1))
}

/// Visible previous/next cover controls immediately after the provenance and
/// candidate count, where the eye already is.
pub fn cover_controls(
    area: Rect,
    show_cover: bool,
    cell_aspect: f32,
    album: Option<&Album>,
    ...
    if album.is_none_or(|a| a.choices < 2) {
        return None;
    }
    let g = geometry(area, show_cover, cell_aspect)?;
    if g.text.width < 5 {
    ...
    let rows = lines(album, fallback_album, fallback_artist);
    let top = g.text.y + (g.text.height.saturating_sub(rows.len() as u16)) / 2;
    let index = rows.iter().position(|(_, k)| *k == Kind::Provenance)?;
    let y = top + index as u16;
    let text = truncate(&rows[index].0, g.text.width.saturating_sub(4) as usize);
    let x = g.text.x + text.chars().count() as u16 + 1;
    Some((Rect::new(x, y, 1, 1), Rect::new(x + 2, y, 1, 1)))
}
...
impl<'a> Widget for AlbumView<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (i, (line, kind)) in rows.iter().enumerate() {
            ...
            buf.set_string(g.text.x, y, truncate(line, available), style);
        }

        if let Some((previous, next)) = cover_controls(
            area,
            self.show_cover,
            self.cell_aspect,
            self.album,
            ...
            buf.set_string(previous.x, previous.y, "<", style);
            buf.set_string(next.x, next.y, ">", style);
        }
...
mod tests {
    ...
    #[test]
    fn multiple_images_get_distinct_previous_and_next_controls() {
        let mut album = album_with(Some(detail()), None);
        album.source = Some(Source::Embedded);
        album.choices = 3;
        album.labels = vec!["embedded".into(), "front.jpg".into(), "back.jpg".into()];
        let area = Rect::new(0, 0, 60, PANEL_ROWS);
        let (previous, next) =
            cover_controls(area, true, CELL_ASPECT, Some(&album), None, None).unwrap();
        assert_eq!(previous.width, 1);
        assert_eq!(next.width, 1);
        assert!(previous.x + previous.width < next.x);
        let screen = draw(Some(album), 60, PANEL_ROWS);
        assert_eq!(
            screen[previous.y as usize].chars().nth(previous.x as usize),
            Some('<')
        );
        assert_eq!(
            screen[next.y as usize].chars().nth(next.x as usize),
            Some('>')
        );
    }

    #[test]
    fn title_and_artist_have_separate_database_hit_targets() {
        let album = album_with(Some(detail()), None);
        let area = Rect::new(0, 0, 60, PANEL_ROWS);
        let title = catalog_rect(
            area,
            true,
            CELL_ASPECT,
            Some(&album),
        ...
        .unwrap();
        assert_ne!(title.y, artist.y);
        assert!(title.width > 0 && artist.width > 0);
    }

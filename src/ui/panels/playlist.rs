...
pub struct Rows {
    ...
    /// Tracks whose album group contains more than one distinct tagged disc.
    multi_disc: Vec<bool>,
}
...
impl Rows {
    fn grouped_now(&self) -> bool {
        self.rows
            .iter()
            .any(|row| matches!(row, Row::Section { .. }))
    }
    ...
    pub fn grouped(items: &[QueueItem], folded: &HashSet<String>, playing: Option<usize>) -> Rows {
        ...
        let mut multi_disc = vec![false; items.len()];
        ...
        while i < items.len() {
            ...
            while end < items.len() && keys[end] == keys[i] {
                year = match (year, items[end].timeline_year.or(items[end].year)) {
                    (Some(a), Some(b)) => Some(a.min(b)),
                    (a, b) => a.or(b),
                };
                end += 1;
            }
            let discs: HashSet<u32> = items[i..end]
                .iter()
                .filter_map(|item| item.disc_no)
                .collect();
            if discs.len() > 1 {
                multi_disc[i..end].fill(true);
            }
    ...
    /// First track belonging to a heading, including when that album is folded.
    pub fn track_for_section(&self, row: usize) -> Option<usize> {
        self.section_of
            .iter()
            .position(|section| *section == Some(row))
    }
...
impl<'a> Widget for PlaylistView<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for row in 0..height {
            ...
            let playlist_idx = format!("{:>4}{}", i + 1, if is_tagged { '+' } else { '.' });
            let album_idx = show_album_indices.then(|| {
                let multiple_discs = self.rows.multi_disc.get(i).copied().unwrap_or(false);
                match (multiple_discs, item.disc_no, item.track_no) {
                    (true, Some(disc), Some(track)) => format!("{disc}.{track:02}"),
                    (_, _, Some(track)) => format!("{track:02}"),
                    (true, Some(disc), None) => format!("{disc}.--"),
                    _ => "--".into(),
                }
            });
            let idx_w = playlist_idx.len()
                + album_idx
                    .as_ref()
                    .map(|album_idx| 3 + album_idx.len().max(5))
                    .unwrap_or(0);
            ...
            let head = format!("{marker}{playlist_idx}");
            ...
            if let Some(album_idx) = album_idx {
                buf.set_string(x, y, " ", style);
                x += 1;
                let separator_bg = bg.unwrap_or(t.panel_bg);
                let mut separator_style = Style::default()
                    .fg(rgb(separator_bg.lighten(0.18)))
                    .bg(rgb(separator_bg));
                if is_playing {
                    separator_style = separator_style.add_modifier(Modifier::BOLD);
                }
                buf.set_string(x, y, "│", separator_style);
                x += 1;
                let album_idx = format!(" {album_idx:>5}");
                buf.set_string(x, y, &album_idx, part(t.row_index_fg));
                x += album_idx.chars().count() as u16;
            }
            buf.set_string(x, y, " ", style);
            x += 1;
...
mod render_tests {
    ...
    #[test]
    fn album_rows_separate_playlist_position_from_disc_and_track_number() {
        let mut items = grouped_items();
        items[0].disc_no = Some(2);
        items[0].track_no = Some(3);
        items[1].disc_no = Some(1);
        let drawn = grouped_draw(&items, &open(&items), 0, 12);
        assert!(
            drawn.iter().any(|line| line.contains("1. │  2.03")),
            "album numbering was not drawn distinctly: {drawn:#?}"
        );

        let flat = grouped_draw(&items, &Rows::flat(items.len()), 0, 12);
        assert!(
            flat.iter().all(|line| !line.contains(" │  2.03")),
            "plain playlists should retain their compact numbering"
        );
    }

    #[test]
    fn a_single_disc_album_does_not_repeat_the_disc_number() {
        let mut items = grouped_items();
        for item in &mut items[..3] {
            item.disc_no = Some(1);
        }
        let drawn = grouped_draw(&items, &open(&items), 0, 12);
        assert!(drawn.iter().any(|line| line.contains("1. │    01")));
        assert!(drawn.iter().all(|line| !line.contains("│  1.01")));
    }
...
mod tests {
    ...
    fn scrolling_follows_the_cursor_up() {
        ...
        assert_eq!(
            V::clamp_scroll(12, usize::MAX, 10),
            12,
            "a forced reveal anchors the cursor at the top"
        );
    }
